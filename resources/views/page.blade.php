...
@section('canonical', $page->canonical_url ?: url('/' . $page->slug))
...
{{-- hreflang اختصاصی: اسلاگِ EN/TRِ یک صفحه لزوماً یکی نیست (ادمین می‌تواند اسلاگِ ترجمه را بومی
     کند، مثل contact ↔ iletisim)، پس نمی‌توان مثل بلوکِ پیش‌فرضِ master صرفاً پیشوندِ /tr را
     اضافه کرد. اگر ترجمه‌ای ثبت نشده باشد، اصلاً ادعای hreflang به زبانِ دیگر نمی‌کنیم — همان
     الگوی blog-post.blade.php --}}
@php($translation = $page->translation ?: $page->translations->first())
@section('hreflang')
<link rel="alternate" hreflang="en" href="{{ url($page->path()) }}">
@if($translation)
...
<style>
    ...
    /* ===== فرم تماس — فقط روی صفحه‌ی contact رندر می‌شود (نگاه کنید به شرط پایین) ===== */
...
<div class="site-page">
    <div class="wrap">
        ...
        <div class="site-page__box">
            ...
            @php($contactSettings = \App\Models\SiteSetting::whereIn('key', ['footer.en.contact_email', 'footer.en.contact_phone', 'footer.en.contact_address'])->pluck('value', 'key'))
            @php($contactEmail = $contactSettings['footer.en.contact_email'] ?? null)
            @php($contactPhone = $contactSettings['footer.en.contact_phone'] ?? null)
            @php($contactAddress = $contactSettings['footer.en.contact_address'] ?? null)
            ...
            <form class="contact-form reveal js-contact-form" method="post" action="{{ url('/contact') }}" novalidate
                  data-msg-toomany="{{ __('contact.too_many', [], 'en') }}"
                  data-msg-error="{{ __('contact.error', [], 'en') }}">
...
@section('page-js')
<script>
    // ===== فرم تماس — همون الگوی AJAX فرم خبرنامه (master.blade.php) =====
    (function () {
