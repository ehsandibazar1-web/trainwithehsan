...
{{-- hreflang اختصاصی: اسلاگ EN/TR یک مقاله لزوماً یکی نیست (مثلاً combat-intelligence در برابر
     ...
     برای این مقاله ثبت نشده باشد، اصلاً ادعای hreflang به زبان دیگر نمی‌کنیم (بهتر از لینک‌دادن
     به یک اسلاگ حدسی که وجود ندارد) --}}
@php($translation = $article->translation ?: $article->translations->first())
@section('hreflang')
<link rel="alternate" hreflang="en" href="{{ url($article->path()) }}">
@if($translation)
<link rel="alternate" hreflang="tr" href="{{ url($translation->path()) }}">
@endif
...
<style>
    ...
    {{-- الان یک <ul> واقعی است (به‌جای <fieldset>/<legend> نامعتبر) — list-style/padding ریست می‌شود --}}
    ...
    {{-- #999 روی پس‌زمینه‌ی سفید کنتراست ~۲.۸۵:۱ داشت — زیرِ آستانه‌ی WCAG AA (۴.۵:۱) برای متنِ عادی --}}
    ...
    <div class="wrap">
        ...
        <div class="site-blog-post__box">
            <div>
                ...
                <div class="share-box reveal">
                    ...
                    <div class="share-buttons">
                        <a href="https://t.me/share/url?url={{ urlencode(url('/blog/' . $article->slug)) }}&text={{ urlencode($article->title) }}" target="_blank" rel="noopener" class="share-btn share-btn-tg">Telegram</a>
                        <a href="https://wa.me/?text={{ urlencode($article->title . ' ' . url('/blog/' . $article->slug)) }}" target="_blank" rel="noopener" class="share-btn share-btn-wa">WhatsApp</a>
                        <button class="share-btn share-btn-cp" onclick="copyLink(this)" data-url="{{ url('/blog/' . $article->slug) }}">Copy link</button>
                    </div>
                </div>
                ...
                <h2 style="border-bottom:4px solid var(--gold);padding:0 0 8px;font-size:17px;font-weight:600;letter-spacing:.01em;color:#000;margin:0">Related Articles</h2>
            ...
            <aside>
                ...
                <h2 style="border-bottom:4px solid var(--gold);padding:0 0 8px;font-size:17px;font-weight:600;letter-spacing:.01em;color:#000;margin:0">Latest Articles</h2>
