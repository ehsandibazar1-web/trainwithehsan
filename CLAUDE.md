...
- **Instagram Showcase (2026-07-17, extended to two rows the same day)** — the homepage's old two static phone-mockup image bands (`insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image`, still preserved in `HomepageSettings::FILE_KEYS` and still saved/loaded, but no longer rendered by any public template) were replaced with premium embedded-Instagram-post/reel cards, per an explicit, detailed user request. This is a pure **extension** of the existing `HomepageSettings` page — no new Filament resource/page, no new CMS module, no renamed/moved/deleted fields. **Row 1** (the original build): six `TEXT_KEYS` (`insta_showcase_enabled`, `insta_embed_url`, `insta_showcase_title`, `insta_showcase_subtitle`, `insta_showcase_button_text`, `insta_showcase_button_url`) and one `FILE_KEYS` entry (`insta_showcase_fallback_image`) appended to the existing constants — `mount()`/`save()`'s generic per-key loops picked them up with zero changes to that logic. **Row 2** (added the same day, per a follow-up request referencing the reference site `ehsandibazar.com`, which shows two independent Instagram bands for two different pages/accounts): a mirrored field set with an `insta_showcase2` prefix (`insta_showcase2_enabled`, `insta_showcase2_embed_url`, `insta_showcase2_title`, `insta_showcase2_subtitle`, `insta_showcase2_button_text`, `insta_showcase2_button_url`, `insta_showcase2_fallback_image`) — both rows are built by the same `instaShowcaseFields(string $l, string $prefix = 'insta_showcase', ?string $embedKey = null, bool $alwaysVisible = true)` helper (Row 1's irregular historical key `insta_embed_url`, without a `_showcase_` infix, is preserved exactly via the `$embedKey` override; Row 2 uses the more regular `insta_showcase2_embed_url`), rendered as two `Section::make(...)` blocks per locale (collapsed by default, "Row 2 (optional)" explicitly labeled as optional in its description). **Row 1 always renders on the public page** (shows its fallback card when disabled — the original, unchanged behavior) while **Row 2 is entirely absent from the DOM unless `insta_showcase2_enabled` is explicitly turned on** — this asymmetry is deliberate: it's what makes Row 2 fully backward-compatible, so every homepage that only ever configured Row 1 renders byte-for-byte identically after this change. Each row's Button URL falls back to the pre-existing `insta_url` field (reusing it, not duplicating it), and that in turn falls back to a hardcoded `https://instagram.com`. On the public side (`home.blade.php`/`tr/home.blade.php`), both rows are built from one `$instaRows` array of per-row data looped with `@foreach`/`@continue` (not two hand-duplicated blocks) — the embed is Meta's **official** `instagram-media` blockquote + `embed.js` widget (no third-party library) for both rows, and the existing lazy-load JS (`document.querySelectorAll('.js-insta-embed').forEach(...)`, unchanged) already generalized to any number of embed cards on the page, so adding Row 2 required zero JS changes. Row 2 gets an alternating white background (`.insta-showcase--row2{background:#fff;border-top:0}`) against Row 1's gray, matching the pre-redesign two-band bg pattern (`.inst2`/`.inst`) and the reference site. The embed card's reserved frame is `max-width:360px;min-height:640px` (~9:16, portrait — matching Instagram's own Reel/Post proportions and the reference site's phone-mockup shape), shared by both rows via the same `.insta-embed-wrap` class — an explicit widening-then-narrowing: the original single-row build initially shipped at `max-width:550px;min-height:460px` (closer to square) and was corrected to the current 9:16 frame the same day, per direct user feedback. The showcase logo above the section title (`.insta-showcase-text .insta-showcase-logo`) is `100px` wide (bumped up from an initial `56px`, matching closer to the `120px` this same logo file uses elsewhere on the site) with explicit `margin-left/right:auto` scoped to the mobile breakpoint — needed because a global `img{display:block}` reset defeats `text-align:center`'s ability to center a block-level image on its own. Changing which post/reel either row shows is a one-field edit (its Embed URL) in the CMS, no code/deploy required — this applies identically to both rows.
...
- **The homepage Instagram Showcase (2026-07-17, see Section 6) was built as a pure extension of `HomepageSettings`, per an explicit, detailed user instruction — not a default architectural choice.** The request enumerated exactly what must NOT happen: no new Homepage resource, no new Instagram/Social Media module, no new CMS page, no rebuilding/replacing `HomepageSettings`, no renaming/deleting/moving any existing field. It also specified the embed must be Meta's **official** embed only (no third-party widget), and that the old `insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image` fields must keep working even though the two static photo bands they fed are gone from the public template — they remain fully intact, saved, and loaded in `HomepageSettings::FILE_KEYS`, simply orphaned from rendering. Do not remove them or "clean them up" as dead code — see Things That Must Never Be Changed. **The same day, the user asked for a second, independent Instagram row (referencing `ehsandibazar.com`, which shows two bands for two different Instagram pages) — this was implemented as `insta_showcase2_*` fields mirroring Row 1 exactly, off by default so Row 1-only configurations render unchanged, and a near-9:16 portrait embed frame (also explicitly requested, "تصویر اینستاگرام نزدیک ۹ در ۱۶ حالت عمودی هست") replacing the original build's more square `550×460` frame.** Do not collapse the two rows back into one, and do not widen the embed frame back toward square without a new explicit request.
...
- Do not build the Instagram Showcase (see Section 6) as a new Filament resource/page/CMS module — it must stay purely an extension of the existing `HomepageSettings` page (new keys in `TEXT_KEYS`/`FILE_KEYS` + collapsed `Section`s, one per row), per the explicit "do NOT create a new Homepage resource / Instagram module / CMS page" instruction behind this feature. Do not rename, move, or delete the pre-existing `insta_url`/`insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image` fields — they must keep saving/loading exactly as before even though the two old image bands are no longer rendered on the public homepage. Do not use a third-party embed widget for the Instagram card — only Meta's official `instagram-media` blockquote + `embed.js`. Do not make `embed.js` load unconditionally or eagerly — it must stay behind both each row's own `*_enabled`/`*_embed_url` check and the shared `IntersectionObserver` lazy-load gate (see Core Web Vitals Rules), and must always degrade to that row's fallback card (image/icon/"Watch on Instagram" button) rather than a blank space or a stuck spinner if the embed fails or times out. Do not make Row 2 (`insta_showcase2_*`) render when `insta_showcase2_enabled` is falsy — unlike Row 1, which always renders (fallback card when disabled, the original behavior), Row 2 must stay entirely absent from the DOM by default so every homepage that only configured Row 1 keeps rendering identically after this addition. Do not revert `.insta-embed-wrap`'s `max-width:360px;min-height:640px` (~9:16) frame back to the original build's more square `550×460` sizing — the narrower portrait frame was an explicit, direct user correction referencing the reference site's phone-mockup proportions.
