...
- **Instagram Showcase (2026-07-17)** — the homepage's old two static phone-mockup image bands (`insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image`, still preserved in `HomepageSettings::FILE_KEYS` and still saved/loaded, but no longer rendered by any public template) were replaced with one premium embedded-Instagram-post/reel card, per an explicit, detailed user request. This is a pure **extension** of the existing `HomepageSettings` page — no new Filament resource/page, no new CMS module, no renamed/moved/deleted fields. Six new `TEXT_KEYS` (`insta_showcase_enabled`, `insta_embed_url`, `insta_showcase_title`, `insta_showcase_subtitle`, `insta_showcase_button_text`, `insta_showcase_button_url`) and one new `FILE_KEYS` entry (`insta_showcase_fallback_image`) were appended to the existing constants — `mount()`/`save()`'s generic per-key loops picked them up with zero changes to that logic — plus a new `Section::make('… — Instagram Showcase')` (collapsed by default, positioned right after the existing "Instagram" section, per locale) built by a new `instaShowcaseFields()` helper, following the exact same per-locale-helper-method pattern as `heroFields()`/`videoFields()`/etc. The Button URL falls back to the pre-existing `insta_url` field (reusing it, not duplicating it), and that in turn falls back to a hardcoded `https://instagram.com` — so an admin who has only ever set the old `insta_url` field still gets a working Follow button with zero further configuration. On the public side (`home.blade.php`/`tr/home.blade.php`), the embed is Meta's **official** `instagram-media` blockquote + `embed.js` widget (no third-party library), lazy-loaded via `IntersectionObserver` only when `insta_showcase_enabled` is on and `insta_embed_url` is set — the embed script is never requested at all otherwise, satisfying "load the script only if this section is enabled." A `min-height:460px` placeholder reserves layout space before the embed (or the fallback) renders, and an 8-second timeout forces the fallback UI (optional fallback image + Instagram icon + "Watch on Instagram" button) if `embed.js` never loads/fails — see Core Web Vitals Rules. Changing which post/reel is shown is a one-field edit (`Instagram Embed URL`) in the CMS, no code/deploy required.
...
- Do not build the Instagram Showcase (see Section 6) as a new Filament resource/page/CMS module — it must stay purely an extension of the existing `HomepageSettings` page (new keys in `TEXT_KEYS`/`FILE_KEYS` + one new collapsed `Section`), per the explicit "do NOT create a new Homepage resource / Instagram module / CMS page" instruction behind this feature. Do not rename, move, or delete the pre-existing `insta_url`/`insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image` fields — they must keep saving/loading exactly as before even though the two old image bands are no longer rendered on the public homepage. Do not use a third-party embed widget for the Instagram card — only Meta's official `instagram-media` blockquote + `embed.js`. Do not make `embed.js` load unconditionally or eagerly — it must stay behind both the `insta_showcase_enabled`/`insta_embed_url` check and the `IntersectionObserver` lazy-load gate (see Core Web Vitals Rules), and must always degrade to the fallback card (image/icon/"Watch on Instagram" button) rather than a blank space or a stuck spinner if the embed fails or times out.
