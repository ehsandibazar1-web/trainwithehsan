...
app/
  ...
  Http/Controllers/
    BlogController.php                       Home, blog index, blog show — EN and TR variants
  ...
  Models/
    ...
    SiteSetting.php                          Generic key/value store — the ad hoc CMS backend for homepage/menu content
...
resources/
  views/
    ...
    filament/pages/                           Custom Filament page Blade views (homepage-settings, menu-settings, editorial-calendar, activity-log)
...
- Custom (non-CRUD) admin screens are Filament **Pages**, not Resources — `EditorialCalendar`, `HomepageSettings`, `MenuSettings`, `ActivityLogPage` are all `Pages` with a paired Blade view under `resources/views/filament/pages/`. Use this pattern for any new admin screen that isn't a straightforward CRUD resource.
- `HomepageSettings` and `MenuSettings` manually decode/encode JSON and normalize Filament's `FileUpload` return shape (e.g. `array_values(array_filter($value))[0] ?? null`) in `mount()`/`save()`. This is brittle by nature (tied to Filament's current return shape) — if a Filament upgrade changes `FileUpload`'s value shape, check these two files first.
...
- JSON-LD structured data has a dedicated `@yield('json-ld')` slot in the master layout, but it is not populated on any page yet. When adding structured data (Article, LocalBusiness, BreadcrumbList, etc.), inject it through this slot — do not hand-write `<script type="application/ld+json">` blocks elsewhere in the templates.
- `og:image` is not currently set anywhere — add it per-page (using the article's `image_path` for blog posts) before relying on social sharing previews looking correct.
...
- Populate the JSON-LD `@yield('json-ld')` slot with `Article` structured data (headline, datePublished, author, image) on `blog-post.blade.php` / `tr/blog-post.blade.php`, and `LocalBusiness`/`Person` structured data on the home/about pages — this is unimplemented today and is one of the highest-leverage additions for AI-answer-engine visibility.
