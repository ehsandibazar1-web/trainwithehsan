...
**Google Analytics 4 and Microsoft Clarity are live (added 2026-07-17), gated behind a cookie-consent banner — this is real, shipped behavior, not a plan.** Google Search Console is also connected, but separately, via a DNS TXT record (Cloudflare) — it needs no code in this repo at all, since Search Console verification and data collection happen entirely outside the site's HTTP responses.

- **Configuration**: two env vars, `GOOGLE_ANALYTICS_ID` (a GA4 Measurement ID, `G-XXXXXXXXXX`) and `MICROSOFT_CLARITY_ID` (a Clarity project ID), read via `config('services.google_analytics.id')` / `config('services.microsoft_clarity.id')` (`config/services.php`). Both are public identifiers embedded in page source by design — not secrets — but are still environment-driven (not hardcoded in Blade) so staging/local installs don't fire the production property by default. **Leaving both blank disables the feature completely**: no banner renders, no tracking script (not even the inert function definitions) is sent to the browser at all — the entire consent/tracking `<script>` block in both layouts is wrapped in `@if($gaId || $clarityId)`, not just the banner's HTML.
- **Cookie-consent banner (KVKK/GDPR) — the explicit, confirmed requirement: no tracking script loads before a visitor accepts.** `layouts/master.blade.php` (English) and `layouts/master-tr.blade.php` (Turkish) each render a fixed-bottom banner (`#cookieConsent`, `hidden` by default in the HTML to avoid a flash-of-banner) with "Accept"/"Decline" buttons (Turkish: "Kabul Et"/"Reddet"), plus a "Learn more" link to `/privacy-policy` (`/tr/privacy-policy` on the Turkish layout). A small vanilla-JS IIFE (same "plain `<script>` at the bottom of body" convention every other piece of site-wide JS in these two files already uses — no new pattern) checks `localStorage['twe_cookie_consent']` on load:
  ...
  - Clicking **Accept** sets `localStorage` to `'accepted'`, hides the banner, and loads both configured trackers (GA4 via a dynamically-created `<script src="https://www.googletagmanager.com/gtag/js?id=...">` + the standard `gtag('js', ...)`/`gtag('config', ...)` calls; Clarity via Microsoft's own official inline-snippet pattern, copied verbatim). Clicking **Decline** sets `localStorage` to `'declined'` and hides the banner — nothing is ever loaded.
  - Both loader functions are idempotent (`window.__gaLoaded`/`window.__clarityLoaded` guards) so a returning visitor with `'accepted'` already stored never double-loads either script.
- **Search Console needs no template code.** Verified via a DNS TXT record added directly at the DNS provider (Cloudflare) for the "Domain" property type — this covers the entire domain (`trainwithehsan.com` and every subdomain/path, including `/tr/...`) automatically, the same as GA4/Clarity each need only one property/project for the whole bilingual site (no separate EN/TR setup for any of the three).
- **Core Web Vitals**: both scripts load `async` (GA4's own `<script>` tag; Clarity's official snippet already sets `t.async=1`), and neither is even requested until the visitor clicks Accept (or, for a returning consenting visitor, after `DOMContentLoaded` via the same bottom-of-body script — never render-blocking, never in `<head>`). This preserves the "no heavy blocking JS" posture from Core Web Vitals Rules below.
- Rules going forward:
  - **Never remove or modify the cookie-consent gate** — GA4/Clarity must never be wired to load unconditionally on page load; that would defeat the explicit KVKK/GDPR requirement this was built for.
  ...
  - If a **Meta Pixel** or any other tracking service (Hotjar, Plausible, Fathom, etc.) is requested later, wire it through this same consent-gated pattern (add its ID to `config/services.php`, load it only inside the same `loadTrackers()` function, guarded by the same `@if`) — do not add a second, differently-gated tracking mechanism.
  - `tests/Feature/AnalyticsTrackingTest.php` covers: nothing renders when both env vars are blank (no banner element, no `googletagmanager.com`/`clarity.ms` reference anywhere in the response, including inside otherwise-inert JS), the banner renders `hidden` by default when at least one is configured (either GA4 alone or Clarity alone is sufficient to render it), the correct IDs are embedded, and the EN/TR banner text/links differ correctly per locale.
...
- **GA4/Microsoft Clarity's cookie-consent gate was an explicit user choice, confirmed via `AskUserQuestion`, not a default this project happened to build.** The user was asked whether a consent banner should exist before tracking fires (given the site targets Turkish/KVKK and EU/GDPR visitors) and confirmed "yes, add it" — this is why the entire tracking `<script>` block, not just the banner's visibility, is gated behind `@if($gaId || $clarityId)` and a `localStorage`-backed accept/decline decision (see Analytics & Tracking). Do not simplify this into an always-on snippet as a "cleanup."
...
- Do not make GA4/Microsoft Clarity load unconditionally on page load — both must stay behind the cookie-consent banner's `localStorage['twe_cookie_consent'] === 'accepted'` gate (Analytics & Tracking) — this was an explicit KVKK/GDPR compliance requirement, not a default the code happened to end up with. Do not remove the `@if($gaId || $clarityId)` wrapper around the entire consent/tracking `<script>` block in `master.blade.php`/`master-tr.blade.php` — an unconfigured install must send zero tracking-related code to the browser, not just hide the banner.
