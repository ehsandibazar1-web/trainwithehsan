...
app/
  ...
  Http/Controllers/
    ...
    ContactController.php                    POST /contact — the Contact page's form (AJAX), same honeypot/time-gate/throttle pattern as NewsletterController, see Section 33
  ...
  Mail/
    ...
    ContactMessageMail.php                   Sent to the site's configured contact_email (FooterSettings) when the Contact page's form is submitted; reply-to is the visitor's own email — see Section 33
  Models/
...
- **Training Articles background is a parallax (fixed-attachment) background, `.section-news` only (2026-07-17)** — per the user's own video of `ehsandibazar.com`'s Articles section plus a screenshot of that site's actual `template.css` (`.section-news{background:url("../images/bg-news.jpg") no-repeat fixed;background-size:cover!important}`), `.section-news`'s background rule gained `background-attachment:fixed` (`center/cover no-repeat fixed`) in both `home.blade.php`/`tr/home.blade.php` — the classic CSS "parallax background" technique (the image stays visually fixed to the viewport while page content scrolls over it). **This is deliberately scoped to `.section-news` alone, per the user's explicit correction** ("نه به هر ۴ بخش اضافه نکن فقط بخش مقالات" — do not add it to all 4 sections, only Articles) — `.counter`, `.result-section`, `.insta-showcase`, and `.insta-showcase--row2` keep their normal `scroll` attachment and must not be changed to match "for consistency." Verified via Playwright's `getComputedStyle(...).backgroundAttachment`: `.section-news` reports `fixed`, the other three report `scroll`. Known, accepted limitation of the technique itself (not special-cased): iOS Safari silently ignores `background-attachment:fixed` and degrades to normal `scroll` — this is standard, expected behavior for this CSS property, not a bug to work around.
...
- **SEO Center** (`SeoCenter` Filament page, nav item "SEO Center"): a read-only audit dashboard over everything described above — it does not replace or duplicate any of the mechanics in this section, it only *reports* on the data those mechanics already consume (`Article`/`Page` rows, `SiteSetting`-driven menu/footer, the Media Library). All checks live in `App\Services\Seo\SeoAuditService` (fast, DB-only checks — titles/descriptions/canonicals/schema/duplicates/internal-links/orphans, re-run automatically every time the page loads) and its `checkExternalLinks()` method (real HTTP HEAD requests via `Http::pool()`, deliberately **not** run automatically — only on the "Scan external links" button — since it's the one check with real network latency). `App\Services\Seo\HtmlContentScanner` is the shared DOMDocument-based helper that both link-checking and ALT-text checks use to read `<a href>`/`<img alt>` out of `Article.body`/`Page.body`. Concretely, per category:
  ...
  - *Missing Schema*: **as of 2026-07-17, `Article` is never flagged (unconditional `Article` schema in `blog-post.blade.php`), and `Page` is also never flagged** (unconditional `WebPage`/`FAQPage` schema in `page.blade.php`/`tr/page.blade.php` — see Section 7/33) — only the blog index (both locales) is still flagged, since `blog.blade.php`/`tr/blog.blade.php` genuinely emit no JSON-LD. `SeoAuditService::missingSchema()` no longer loops over `Page` rows at all for this category (it used to flag every single one) — if this check is ever seen flagging a `Page` again, that's a regression, not a re-introduced gap.
...
## 33. Important Project Decisions
...
- **`SeoAuditService::missingSchema()` no longer flags `Page` rows, because `page.blade.php`/`tr/page.blade.php` now genuinely emit JSON-LD (`WebPage`, plus `FAQPage` when `faqs` is set) — this closes a gap the audit existed specifically to surface (Section 8/9's Future Development Guidelines item 4), it does not weaken the check.** Only the blog index (`blog.blade.php`/`tr/blog.blade.php`) is still flagged, since that template genuinely has no JSON-LD yet. The AI Agent dashboard's `missing_schema` category (Section 29, a thin wrapper over this same service) inherited this fix automatically with no code change of its own.
...
## 34. Things That Must Never Be Changed
...
- Do not re-introduce a hardcoded list of static slugs in `SeoController::sitemap()` for standalone pages — it loops every published `Page` record (`Page::published()->get()`) exactly like `Article`, specifically so a newly published page never needs a code change to appear in the sitemap. Do not let `SeoAuditService::missingSchema()` start flagging `Page` rows again without first checking whether `page.blade.php`/`tr/page.blade.php` still emit `WebPage` schema — that's the only reason the flag was removed.
...
- Do not add `background-attachment:fixed` (the parallax effect) to `.counter`, `.result-section`, `.insta-showcase`, or `.insta-showcase--row2` — the user explicitly asked for this effect on the Training Articles section (`.section-news`) only ("نه به هر ۴ بخش اضافه نکن فقط بخش مقالات") and explicitly rejected applying it to all four background sections for visual consistency. If asked to extend it to another section later, that needs a fresh explicit request, not an inference from "the other backgrounds look similar."

## 35. Future Development Guidelines
