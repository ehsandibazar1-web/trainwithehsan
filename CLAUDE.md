...
app/
  ...
  Filament/
    ...
    Pages/HomepageSettings.php               Homepage content-block editor (per locale) — includes the Instagram Showcase fields (`insta_showcase_*`) added 2026-07-17, see Section 6
    ...
    Pages/MediaLibrary.php                   Digital Asset Management (DAM): folders, drag-and-drop multi-upload, search/filters, preview, ALT/usage tracking, replace, delete-guard — a thin shell over `App\Filament\Concerns\InteractsWithMediaLibrary` (shared with the unified Media Picker), see Section 31
    Concerns/InteractsWithMediaLibrary.php   Shared Media Library mechanics (folders/upload/filters/detail/delete/regenerate) used by BOTH the MediaLibrary page and the MediaPicker modal — one implementation, not two, see Section 31
    Forms/Components/MediaPickerInput.php     The single media/file field used everywhere (featured image, hero, all Homepage/About/Footer fields, repeater rows, video files, RichEditor insert); stores a plain `disk_path` string, opens the unified Media Picker, see Section 31
    ...
    Pages/AiActionRouting.php                Global default/failover provider + per-ActionRegistry-field provider override, grouped under collapsible sections (SEO/Content/Translation/Media/Links/Schema), plus an "Embeddings" section picking the active RAG embedding provider (`ai_provider_settings.embedding_provider_config_id`, OpenAI/Gemini only) and an "Image Generation" section picking the default/failover/fallback image provider (`ai_provider_settings.default_image_provider_config_id`/`image_failover_enabled`/`fallback_image_provider_config_id`, OpenAI/Gemini only) — see Sections 24, 27, and 30
  ...
  Http/Controllers/
    ...
    PageController.php                       Standalone pages (privacy, terms, cookie-policy, disclaimer, faq, contact, ...) — EN and TR show routes
    NewsletterController.php                 Newsletter subscribe (AJAX) / verify / unsubscribe / resend — honeypot, time-gate, rate limit
    ContactController.php                    POST /contact — the Contact page's form (AJAX), same honeypot/time-gate/throttle pattern as NewsletterController, see Section 34
    SeoController.php                        sitemap.xml (now includes every published Page, not just Article — see Section 8), /feed, /tr/feed
  ...
  Livewire/MediaPicker.php                   The unified Media Picker modal — mounted once globally (AdminPanelProvider BODY_END render hook), opened by every MediaPickerInput field + the RichEditor insert; uses InteractsWithMediaLibrary, see Section 31
  ...
  Jobs/GenerateHeroImage.php                 Queued — one-click "Generate Hero Image": ProviderManager::generateImage() → MediaProcessor::store() → sets image_path → auto-fills ALT/caption/description (always) and blank SEO fields (only if blank) via the existing ContentAssistantService/AiGeneration/GenerationApplier pipeline; two cancellation checkpoints like RunAiContentGeneration/TranslateArticleDraft — see Section 30
  ...
  Mail/
    ...
    ContactMessageMail.php                   Sent to the site's configured contact_email (FooterSettings) when the Contact page's form is submitted; reply-to is the visitor's own email — see Section 34
  Models/
    Article.php                              Bilingual article model, translation self-relation, published/locale scopes, faqs JSON column (per-article FAQ + FAQPage schema); also `getOptimizedImageUrlAttribute()` (WebP delivery for public templates, via `Media::forRecord()`) — see Section 21
    Page.php                                 Standalone-page model — same bilingual two-row shape as Article, fully separate from blog; also `getOptimizedImageUrlAttribute()`, same as Article — see Section 21. Since 2026-07-17 also carries `meta_keywords`/`canonical_url`/`robots` (extended SEO fields) and `faqs` (JSON, cast to array, same shape/purpose as `Article.faqs`) — see Section 8
    ...
    Media.php                                 Media Library (DAM) record — disk_path/url/mime/size + folder_id, alt_text, width/height, webp_path, thumbnail_path, responsive_paths (JSON); `warnings()` and `usages()` are computed, not stored; also `caption`/`description` (real columns, part of `ActionRegistry::MEDIA_BACKED_FIELDS`) — see Section 30
    ...
    AiProviderConfig.php                      One vendor's connection settings — encrypted `api_key`, `is_usable` computed accessor (enabled AND has a key) — see Section 24; also `embedding_model`/`is_usable_for_embeddings` (OpenAI/Gemini only — `EMBEDDING_CAPABLE_SLUGS`) — see Section 27; and `image_model`/`is_usable_for_image_generation` (same two providers — `IMAGE_GENERATION_CAPABLE_SLUGS`) — see Section 30
    ...
    AiUsageLog.php                             One row per ProviderManager::respond()/embed()/generateImage() call, success or failure — denormalized provider_slug/model (no FK) so deleting a config never breaks usage history; `action_key` is `'embedding'`/`'image_generation'` for those two calls — see Sections 24, 27, and 30
    AiProviderSetting.php                     Singleton settings row (default/failover/fallback provider) — `current()` is the only read path — see Section 24; also `embedding_provider_config_id` (RAG embeddings) — see Section 27; and `default_image_provider_config_id`/`image_failover_enabled`/`fallback_image_provider_config_id` (hero-image generation) — see Section 30
    AiImageGeneration.php                     One "Generate Hero Image" run for one Article/Page — status/provider_slug/model/media_id/error, status can also be `cancelled`; kept separate from AiGeneration since its output is binary/Media-shaped, not text/array-shaped — see Section 30
  ...
  Services/Seo/VideoSchemaService.php        Builds VideoObject JSON-LD for the homepage video row + member videos (`forHomepage()`) AND every in-body video on an Article/Page (`forArticle()`/`forPage()`) — detection reuses EmbedRenderer (single source of truth), also feeds the Video Sitemap; from existing data only, additive, no UI change — Video SEO, see Section 8
  Services/Content/EmbedRenderer.php          Post-sanitize body processor — turns a standalone provider link into a click-to-load embed facade (no <iframe> stored/allowlisted); `extractVideos()` returns the same detected embeds without rendering (for VideoObject schema), see Section 8
  Services/Content/Embeds/                     Modular embed providers (EmbedProvider contract + YouTube/Vimeo/Instagram/TikTok/SelfHosted) — see Section 8
  ...
  Services/AiAssistant/Contracts/EmbeddingProvider.php   Optional interface (embed(), lastEmbeddingUsage()) — implemented only by OpenAiProvider/GeminiProvider — see Section 27
  Services/AiAssistant/Contracts/ImageProvider.php   Optional interface (generateImage(), lastImageUsage()) — implemented only by OpenAiProvider/GeminiProvider, the same two vendors as EmbeddingProvider — see Section 30
  ...
  Services/AiAssistant/Providers/AnthropicProvider.php, OpenAiProvider.php, GeminiProvider.php, GrokProvider.php, DeepSeekProvider.php, NullProvider.php   The five live vendor providers (OpenAI/Grok/DeepSeek share an OpenAiCompatibleProvider base class) plus the no-key-configured fallback — see Section 24. OpenAiProvider/GeminiProvider additionally implement EmbeddingProvider (Section 27) and ImageProvider (Section 30)
  ...
  Services/AiAssistant/ProviderManager.php   Orchestration layer every AI feature calls instead of an AiProvider directly — resolution (override → default → legacy .env), retry, failover, usage logging, cost estimation, testConnection() — see Section 24. Also embed() (Section 27) and generateImage() (Section 30) — the latter replicates respond()'s full multi-candidate failover, unlike embed()'s deliberately-no-failover design
  Services/AiAssistant/ActionRegistry.php    Every generatable field: label, applicable model(s), allowed modes, response shape, prompt instruction — see Section 23; also `MEDIA_BACKED_FIELDS` (`alt_text`/`caption`/`description` — write to `Media`, not the Article/Page record) and the four `hero_image_prompt`/`thumbnail_image_prompt`/`og_image_prompt`/`social_image_prompt` fields — see Section 30
...
routes/
  web.php                                     All public + admin-adjacent routes (see Security Rules — contains two routes that must be fixed). Also `POST /contact` (throttled, the Contact page's form) — registered above the reserved-slug `Page` catch-all, alongside the newsletter POST routes
...
- **Instagram Showcase (2026-07-17, extended to two rows + old section removed the same day)** — the homepage's old two static phone-mockup image bands were replaced with premium embedded-Instagram-post/reel cards, per an explicit, detailed user request. This is a pure **extension** of the existing `HomepageSettings` page — no new Filament resource/page, no new CMS module. **Row 1** (the original build): six `TEXT_KEYS` (`insta_showcase_enabled`, `insta_embed_url`, `insta_showcase_title`, `insta_showcase_subtitle`, `insta_showcase_button_text`, `insta_showcase_button_url`) and one `FILE_KEYS` entry (`insta_showcase_fallback_image`) appended to the existing constants — `mount()`/`save()`'s generic per-key loops picked them up with zero changes to that logic. **Row 2** (added the same day, per a follow-up request referencing the reference site `ehsandibazar.com`, which shows two independent Instagram bands for two different pages/accounts): a mirrored field set with an `insta_showcase2` prefix (`insta_showcase2_enabled`, `insta_showcase2_embed_url`, `insta_showcase2_title`, `insta_showcase2_subtitle`, `insta_showcase2_button_text`, `insta_showcase2_button_url`, `insta_showcase2_fallback_image`). Both rows are built by the same `instaShowcaseFields(string $l, string $prefix = 'insta_showcase', ?string $embedKey = null, bool $alwaysVisible = true, bool $includeProfileUrl = false)` helper, rendered as two `Section::make(...)` blocks per locale (collapsed by default, "Row 2 (optional)" explicitly labeled optional).
  - **The old "Instagram" section and its four orphaned image fields (`insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image`) were REMOVED (2026-07-17, per a later explicit user request — "if it's not depended upon, remove it").** They were no longer rendered by any template and nothing referenced them. They're gone from `instaFields()` (method deleted), from `FILE_KEYS`, and from the `form()`. Existing DB rows for those keys are harmless orphans (never read, never overwritten). **`insta_url` was KEPT** (it *is* depended on — the shared fallback for both rows' Follow buttons and the fallback-overlay "Watch on Instagram" link) and folded into Row 1's Showcase section via `instaShowcaseFields(..., includeProfileUrl: true)`, so there is now exactly one Instagram area in the admin, not two. This reverses this file's earlier "never delete `insta1_image`..." rule, on the user's own instruction.
  - **Row 1's embed key is the irregular historical `insta_embed_url` (no `_showcase_` infix), and the `form()` call MUST pass it explicitly** as `instaShowcaseFields('en', 'insta_showcase', 'insta_embed_url', ...)`. This is not cosmetic: `mount()`/`save()`/the public template all key off `insta_embed_url`, so if the form field is left to the helper's default (`{$prefix}_embed_url` → `insta_showcase_embed_url`), the field the admin types into is written to a dead key that nothing reads and the Row 1 embed silently never saves — a real regression that shipped briefly when the helper was first generalized for Row 2, discovered by the user ("first field's URL didn't work, but the same URL worked in the second field"), and now covered by `tests/Feature/InstaShowcaseSaveTest.php`. Row 2 uses the regular `insta_showcase2_embed_url` (matches its default), so it was never affected.
  - **Row 1 always renders on the public page** (fallback card when nothing is configured — original behavior) while **Row 2 is entirely absent from the DOM until it has real content** — deliberate asymmetry, so a Row-1-only homepage renders identically after this change. On the public side (`home.blade.php`/`tr/home.blade.php`) both rows come from one `$instaRows` array looped with `@foreach`/`@continue` (not duplicated blocks) — the embed is Meta's **official** `instagram-media` blockquote + `embed.js` (no third-party library), and the existing lazy-load JS (`document.querySelectorAll('.js-insta-embed').forEach(...)`, unchanged) already generalized to any number of cards, so Row 2 needed zero JS changes.
  - **The `Enable Instagram Showcase` toggle does NOT gate whether an embed or a fallback image displays (2026-07-17, fixed the same day per user report) — it never did for Row 1, and the earlier Row 2 implementation's stricter gate was a bug, not a feature.** The embed-vs-fallback decision inside a row is now `@if($row['embed_url'])` — full stop, no `enabled` check — matching Row 1's actual behavior since day one. Row 2's *visibility* (whether its `<section>` renders at all) is `@continue(!$row['always_visible'] && !$row['enabled'] && !$row['embed_url'] && !$row['fallback_image'])` — i.e. the row appears the moment it has an Embed URL **or** a Fallback Image **or** the toggle is explicitly on; an untouched install has none of the three, so it still stays hidden by default, preserving the original backward-compatibility guarantee without needing `enabled` as the sole gate. Before this fix, Row 1's fallback-image-only case already worked (Row 1 is always-visible regardless of `enabled`) while Row 2's did not (the whole section was skipped whenever `enabled` was off, even with a Fallback Image set) — and Row 1's embed required `enabled && embed_url` while Row 2's only required `embed_url` was reachable at all once the admin had already flipped Row 2's toggle on to see anything. The user reported this exact asymmetry ("Row 1 shows my uploaded photo but not my video URL; Row 2 shows the video URL but not the photo") and asked that both rows behave identically — they now do, verified by `tests/Feature/InstaShowcaseSaveTest.php`'s `test_row{1,2}_embed_shows_without_the_enabled_toggle_being_on`/`test_row{1,2}_fallback_image_shows_...`/`test_row2_stays_hidden_when_nothing_is_configured`. The `Enable` toggle field itself was kept (its `SiteSetting` key still saves/loads, and it still counts as one of Row 2's three visibility signals) but is now functionally optional for both rows — the Filament field helper text was rewritten accordingly ("Not required — this row always appears...uploaded photo/URL both appear automatically without needing this toggle").
  - **Layout**: `.insta-showcase-grid` is two columns side-by-side down to a **`640px`** stacking breakpoint (lowered from the initial `900px` per user request, so tablets stay side-by-side like desktop and only phones stack single-column). Row 2's columns are **reversed in side-by-side mode** (`@media (min-width:641px){.insta-showcase--row2 .insta-showcase-card{order:1}.insta-showcase--row2 .insta-showcase-text{order:2}}`) — card left / logo-text right, mirroring Row 1's logo-text left / card right, the zigzag the reference site uses; on stacked mobile the DOM order (text first) is preserved for both rows. Row 2 also gets an alternating white background (`.insta-showcase--row2{background:#fff;border-top:0}`) against Row 1's gray. The embed card's reserved frame is `max-width:360px;min-height:640px` (~9:16 portrait — matching Instagram's Reel/Post proportions and the reference site's phone-mockup shape; corrected from the original build's more-square `550×460` per direct user feedback). The text column (`.insta-showcase-text`) is `text-align:center` at **all** breakpoints (logo/icon, title, subtitle, and Follow button stacked and centered, in both the side-by-side and stacked layouts — per an explicit user request; it was originally left-aligned on desktop/tablet and only centered on mobile). The showcase logo above the title is `100px` wide with `margin:0 auto 20px` (a global `img{display:block}` reset otherwise defeats `text-align:center` on a block image, so the explicit auto side-margins are load-bearing, not redundant).
  - Changing which post/reel either row shows is a one-field edit (its Embed URL) in the CMS, no code/deploy required.
- **Homepage section background images/watermark (2026-07-17)** — the Courses (`.counter`), Training Articles (`.section-news`), Member Results (`.result-section`), and both Instagram Showcase rows (`.insta-showcase`/`.insta-showcase--row2`) now use real background images matching `ehsandibazar.com` pixel-for-pixel, per the user directly uploading the five actual image files (this environment's network policy blocks outbound access to `ehsandibazar.com`, so these could not be fetched/inspected directly — see Important Project Decisions). Committed as **static design assets**, not admin-uploaded/DAM content — `public/images/homepage/{bg-courses.jpg, bg-articles.jpg, watermark-shield.png, bg-instagram-row1.jpg, bg-instagram-row2.jpg}`, referenced via `asset('images/homepage/...')` directly inside each view's `<style>` block (Blade interpolates `{{ }}` there exactly like anywhere else in the file) — same "static design images... committed directly under `public/`, out of scope for the DAM" posture as the hero/about/footer images. `.counter` layers a `linear-gradient(rgba(15,15,15,.72),rgba(15,15,15,.82))` on top of `bg-courses.jpg` to guarantee contrast for its white heading text (`.title-counter`/`.sun-counter`), since the photo itself isn't uniformly dark — the exact same "gradient over background photo" technique the hero slider already uses. `.section-news`/`.result-section`/both Instagram rows need **no** overlay: their existing dark text (`--title`/`--text`, `#000`, `#222020`) already has enough contrast against these particular light textures, and every piece of readable content sits inside its own opaque card/pill (`.news-card{background:#fff}`, `.l-title{background:var(--gold)}`, `.insta-showcase-btn{background:#252525}`) regardless. `.result-section`'s watermark (`watermark-shield.png`, a transparent low-opacity PNG of the brand's shield emblem) is positioned `no-repeat right bottom / 260px auto` — not tiled, not stretched — matching the reference site's single large corner watermark. All five files were verified to compile in both `home.blade.php`/`tr/home.blade.php` and checked visually via Playwright for text contrast before shipping.
- **Training Articles background is a parallax effect via native `background-attachment:fixed`, `.section-news` only (2026-07-17, iterated four times the same day — final state below)** — per the user's own video of `ehsandibazar.com`'s Articles section plus a screenshot of that site's actual `template.css` (`.section-news{background:url("../images/bg-news.jpg") no-repeat fixed;background-size:cover!important}`), this section uses the classic CSS "parallax background" technique. **This is deliberately scoped to `.section-news` alone, per the user's explicit correction** ("نه به هر ۴ بخش اضافه نکن فقط بخش مقالات" — do not add it to all 4 sections, only Articles) — `.counter`, `.result-section`, `.insta-showcase`, and `.insta-showcase--row2` are untouched and must not be changed to match "for consistency."
  - **Round 1**: plain `background-attachment:fixed`, matching the reference site's CSS exactly, with the original `bg-articles.jpg` (1349×637px).
  - **Round 2** (same day): the user reported visible jitter/shift when scrolling stopped ("پس زمینه کیفیتش پایینه اسکرول می‌کنی وایمیستی جابه‌جا میشه میلرزه") — native `background-attachment:fixed` forces the browser to repaint the fixed background on every scroll frame (a well-documented Chrome/Windows rendering cost). Replaced with a GPU-composited `transform`-based layer (`.section-news-bg`, `requestAnimationFrame`-throttled scroll JS, `prefers-reduced-motion`-aware) that reproduced the same visual effect without the repaint cost.
  - **Round 3** (same day): once discussing image quality separately, it became clear `bg-articles.jpg` was only 1349×637px — on any monitor wider than that (1920px, 2560px, extremely common), `background-size:cover` had to upscale it past its native resolution, which likely contributed to the perceived "low quality"/shimmer alongside the repaint cost. The user asked to revert the Round-2 transform workaround and go back to the reference site's original, simple `background-attachment:fixed` technique, paired with a higher-resolution `bg-articles.webp` (1822×863px, AI-upscaled by the user from the original JPEG via an external tool). The `.section-news-bg` layer/markup/JS from Round 2 were removed.
  - **Round 4 (current, final state)** — the user directly compared both images and explicitly preferred the *original* JPG's look over the AI-upscaled WebP ("عکس پس زمینه قبلی بهتر بود / همون باشه"), a plain visual/aesthetic preference, not a technical quality judgment. `bg-articles.webp` was deleted (fully unreferenced) and `bg-articles.jpg` (1349×637px) was restored from git history — `.section-news` is back to the exact Round-1 shape: `background:url('.../bg-articles.jpg') center/cover no-repeat fixed;padding:56px 0 64px`, no transform layer, no extra JS.
  - **Do not re-attempt a "higher resolution" swap of this specific image again without the user separately asking for it** — this was already tried (Round 3) and explicitly rejected on pure visual grounds (Round 4), independent of the technical upscaling argument that motivated trying it in the first place. If jitter is ever reported again on this section, the fix that actually stuck was the plain revert to Round-1's exact technique — don't re-reach for the Round-2 transform workaround as a first response.
...
- **Every media/file field (featured image, hero, all Homepage/About/Footer image fields, repeater rows, video files, the RichEditor's insert) uses the one `App\Filament\Forms\Components\MediaPickerInput` field, which opens the unified Media Picker (Section 31).** It stores the same plain `disk_path` string the old `FileUpload` + `->saveUploadedFileUsing(MediaProcessor::store)` did, so `image_path`'s stored format and every existing reader (`BlogController`, blade views, `SeoController`) is unchanged — WebP/thumbnail/responsive are still generated (uploads inside the picker go through `MediaProcessor::store()`), ALT is edited in the picker's preview panel, usage is tracked, delete is guarded. For any new media field, use `MediaPickerInput::make(...)` (add `->onlyImages()` for image-only fields) — do NOT hand-wire a `FileUpload` + `saveUploadedFileUsing`, and do NOT build a searchable-`Select` media picker (the old `MediaLibraryUploads` helpers were removed). `App\Jobs\GenerateHeroImage` (the AI Image Pipeline, Section 30) still calls `MediaProcessor::store()` directly with an AI-generated `UploadedFile` — that programmatic path was never a form field and is untouched.
...
- **`robots` meta has a per-page override, added 2026-07-17.** `master.blade.php`/`master-tr.blade.php` render `<meta name="robots" content="@yield('robots', 'index,follow')">` — any view can `@section('robots', '...')` to override the site-wide `index,follow` default. Standalone `Page` records expose this as a Filament `Select` (`robots` column, nullable — blank means "use the default") in the SEO section of `PageForm`; `page.blade.php`/`tr/page.blade.php` set `@section('robots', $page->robots ?: 'index,follow')`. Prefer `->middleware('signed')` (see `/preview/article/{article}`) over `noindex` for anything that must not be *publicly reachable at all* — `robots` only asks well-behaved crawlers not to index a page that is still publicly loadable.
- **`Page` also has `meta_keywords` (nullable string, rarely used by modern search engines but supported since it was explicitly requested) and `canonical_url` (nullable string override — blank means the computed `url('/'.slug)`/`url('/tr/'.slug)` fallback, unchanged from before).** Both are plain columns added by `2026_07_17_000010_add_extended_seo_fields_to_pages_table.php`, editable in `PageForm`'s existing "SEO & social preview" section — no second SEO system, this is the same section `seo_title`/`meta_description`/`og_title`/`og_description` already lived in.
...
- **`SeoController::sitemap()` now includes every published `Page` record, not just a hardcoded list of slugs (added 2026-07-17).** The 6 static URLs (`/`, `/about`, `/blog`, `/tr`, `/tr/about`, `/tr/blog`) are still hardcoded (Home/About/Blog aren't `Page` records), but standalone pages are looped exactly like `Article` (`Page::published()->get()`, `url($page->path())`, `<lastmod>` from `updated_at`) — so any new `Page` an admin publishes from the panel is automatically in the sitemap with zero code changes. Do not go back to a hardcoded per-slug list.
- JSON-LD structured data has a dedicated `@yield('json-ld')` slot in the master layout, populated on `home.blade.php`/`tr/home.blade.php` (`Organization` + `Person` graph), `about.blade.php`/`tr/about.blade.php` (`Person` schema), `blog-post.blade.php`/`tr/blog-post.blade.php` (`Article` schema, plus `FAQPage` when the article has FAQs), and — **added 2026-07-17** — `page.blade.php`/`tr/page.blade.php` (`WebPage` schema always, plus `FAQPage` when the page's `faqs` column is non-empty, same combination pattern as `blog-post.blade.php`). The blog **index** (`blog.blade.php`/`tr/blog.blade.php`) is the one remaining template with no JSON-LD at all — see "Missing Schema" below and Future Development Guidelines. When adding structured data to a new page, inject it through this slot — do not hand-write `<script type="application/ld+json">` blocks elsewhere in the templates. `home.blade.php`/`tr/home.blade.php` additionally emit **`VideoObject`** JSON-LD (one script per video) inside this same slot, for the homepage video row and member-result videos — built by `App\Services\Seo\VideoSchemaService::forHomepage()` from existing data only (caption/thumbnail/embed-URL/uploaded-file/member-photo; YouTube & Vimeo recognized, YouTube thumbnails derived from the video id, `uploadDate` from the self-hosted file's `Media.created_at`), passed in by `BlogController::home()`/`homeTr()`. It is **purely additive**: a video with no source or no thumbnail is skipped, and a homepage with no videos configured emits no `VideoObject` at all (byte-identical to before).
- **`VideoObject` for in-body Article/Page videos + a Google Video Sitemap (2026-07-18, "C1 + C2" — the last two Video-SEO gaps from the audit).** Every self-hosted/YouTube/Vimeo video that renders as a click-to-load facade inside an Article or Page body now also gets its own `VideoObject` JSON-LD, via `VideoSchemaService::forArticle()`/`forPage()` (passed in by `BlogController::renderShow()` / `PageController::renderShow()`, emitted through the shared `resources/views/partials/video-schema.blade.php` — which `home.blade.php`/`tr/home.blade.php` now use too, so there's exactly one emission path). **Detection is a single source of truth**: the service calls `EmbedRenderer::extractVideos()`/`detect()` (the same providers that build the facade) — it has no independent "is this YouTube" regex, so schema and the actual rendered player can never drift. Schema fields are derived from the provider match: `embedUrl` (`www.youtube.com/embed/{id}` / `player.vimeo.com/video/{id}`, unchanged from the homepage's format), `contentUrl` for self-hosted, `thumbnailUrl` = the YouTube-derived thumb else the record's featured image (`image_path`, the original file — never the WebP, to avoid a `Media::forRecord()` query on every request), `uploadDate` = `published_at`. Instagram/TikTok/audio never become `VideoObject`s (their canonical home is on-platform); a video with no resolvable thumbnail or source is skipped; duplicate sources on one page are de-duped. **Backward-compatible & additive**: an Article/Page with no body video emits nothing new. The **sitemap** (`SeoController::sitemap()`) now declares `xmlns:video` and, for each Article/Page, appends `<video:video>` children (`thumbnail_loc`/`title`/`description`/`player_loc`|`content_loc`/`publication_date`) built from the **same** `forArticle()`/`forPage()` output — one detector powers homepage schema, article/page schema, and the sitemap. No new queries on any request (bodies are already loaded; thumbnail/uploadDate avoid extra lookups) and the privacy-first, no-eager-`<iframe>` facade is untouched — verified in a real browser: zero third-party requests before click, the `youtube-nocookie` player only after click. Covered by `tests/Feature/VideoSeoTest.php`. This closes the audit's Critical items; remaining Video-SEO work (`duration`, Instagram/TikTok schema, `og:video`/`twitter:player`, transcripts/captions, Key Moments) stays deferred — see Future Development Guidelines.
- **In-body media embeds — click-to-load facades, no sanitizer changes.** Article/Page bodies render through `App\Services\Content\EmbedRenderer::render()` **after** `Str::sanitizeHtml()` (in `blog-post.blade.php`/`page.blade.php` and their `tr/` counterparts). It upgrades a **standalone-paragraph link** (`<p>` whose only content is one `<a>`) whose href is a recognized provider into a **click-to-load facade** `<div class="twe-embed" data-embed-kind data-embed-src>` — a deliberately safer design than opening the sanitizer's allowlist: embeds live in the body as ordinary links (which survive TipTap + the sanitizer untouched, no JS build), and the facade is built *after* sanitize from validated data, so **no `<iframe>`/`<video>` is ever stored or allowlisted**. Providers are modular (`App\Services\Content\Embeds\EmbedProvider` — `YouTubeEmbedProvider` & `VimeoEmbedProvider` build a `youtube-nocookie`/`player.vimeo.com` iframe; `InstagramEmbedProvider` & `TikTokEmbedProvider` build the vendor's official `<blockquote>` + `embed.js` on click, not an iframe — `kind` is `instagram`/`tiktok` so the JS knows which to build, and TikTok carries an optional `id`→`data-video-id`; `SelfHostedEmbedProvider` matches same-origin media files only); adding a provider is a new class + one entry in `EmbedRenderer::defaultProviders()`. The facade loads **zero third-party resources (not even a thumbnail) until the visitor clicks** — a small vanilla-JS partial (`resources/views/partials/media-embeds.blade.php`, `@include`d in both master layouts) builds the real iframe / `<video>`/`<audio>` player / Instagram-or-TikTok blockquote+`embed.js` only on click/Enter, in a reserved box (no CLS). Fully backward-compatible: a body with no recognized standalone-link (fast-path guarded by a substring check) returns byte-identical; an inline or non-provider link is left untouched; external (non-same-origin) media links are never embedded. **Note:** `EmbedRenderer` must `html_entity_decode()` the extracted href (the sanitizer encodes `=` as `&#61;`), not `htmlspecialchars_decode()` which leaves numeric entities intact. **Creating an embed:** the RichEditor's "Media Library" Insert modal has an "Or paste a video/embed URL" field (inserts a standalone-paragraph link the renderer detects), and a video/audio file picked from the Media Library is likewise inserted as a standalone link (→ a click-to-load player) instead of a download link; pasting a bare provider URL on its own line in the editor also works. The `Content-Security-Policy-Report-Only` header (`App\Http\Middleware\AddSecurityHeaders`, non-enforcing) lists the embed hosts (`youtube-nocookie.com`/`instagram.com`/`tiktok.com` in `frame-src`; `instagram.com`/`tiktok.com` in `script-src`) so a future enforced CSP won't break embeds — do not drop those entries.
- **The homepage Video Row + member-result videos also use this same engine.** Their `Embed URL` field (YouTube/Vimeo/Instagram/TikTok) is resolved server-side via `EmbedRenderer::detect()` (unknown providers like Aparat fall back to a plain iframe with the raw URL — backward-compatible), and each `.js-video` card carries `data-embed-kind`/`data-embed-src`/`data-embed-id`. Clicking a card opens the existing video modal, which builds the player through the **shared** `window.tweMediaEmbed.build(container, kind, src, id)` helper the partial exposes — so the homepage modal and the in-body facade use one JS builder, not two copies (the modal gets a `--social` variant so portrait Instagram/TikTok fit). Still privacy-first: nothing third-party loads until the visitor clicks a video card. The old per-view `$embed` closure in `home.blade.php`/`tr/home.blade.php` was replaced by an `$embedMatch` closure calling `EmbedRenderer::detect()`.
- **Standalone `Page` records gained an `faqs` JSON column (`2026_07_17_000010...`), mirroring `Article.faqs` exactly** (same cast-to-array, same `{question, answer}` shape, same `Repeater` field in `PageForm`, same `<details>`/`<summary>` accordion rendering and `FAQPage` JSON-LD in the public template) — this is what backs the `/faq` and `/tr/faq` pages (see Section 34) and is available to any other `Page`, not just the FAQ one.
...
- **SEO Center** (`SeoCenter` Filament page, nav item "SEO Center"): a read-only audit dashboard over everything described above — it does not replace or duplicate any of the mechanics in this section, it only *reports* on the data those mechanics already consume (`Article`/`Page` rows, `SiteSetting`-driven menu/footer, the Media Library). All checks live in `App\Services\Seo\SeoAuditService` (fast, DB-only checks — titles/descriptions/canonicals/schema/duplicates/internal-links/orphans, re-run automatically every time the page loads) and its `checkExternalLinks()` method (real HTTP HEAD requests via `Http::pool()`, deliberately **not** run automatically — only on the "Scan external links" button — since it's the one check with real network latency). `App\Services\Seo\HtmlContentScanner` is the shared DOMDocument-based helper that both link-checking and ALT-text checks use to read `<a href>`/`<img alt>` out of `Article.body`/`Page.body`. Concretely, per category:
  ...
  - *Missing Schema*: **as of 2026-07-18, this check always returns zero findings — `Article` (`blog-post.blade.php`), `Page` (`page.blade.php`/`tr/page.blade.php`), and the blog index (`blog.blade.php`/`tr/blog.blade.php`, a `CollectionPage`/`ItemList` added 2026-07-18) all emit real JSON-LD now.** `SeoAuditService::missingSchema()` takes no arguments and unconditionally `return []`s — same "tripwire" pattern as `missingCanonicals()` (see below) — kept in the code and in `run()`'s category list so a future regression in any of the three templates is visible immediately instead of silently losing coverage.
  - *Broken Internal/External Links* and *Orphan Pages*: built from the same internal link graph — `Article.body`, `Page.body`, `menu.{locale}.items[].url`, and `footer.{locale}.columns[].links[].url`. A link is "internal" if its host matches `config('app.url')`'s host (never hardcode the domain — see Things That Must Never Be Changed). Internal target resolution mirrors `routes/web.php` exactly (`SeoAuditService::internalPathExists()`) — if that file's route list changes, update this method too. A published `Article`/`Page` is "orphan" if its own path never appears as a link target anywhere in that same graph; draft/scheduled items are never flagged as orphans (they aren't expected to be linked yet). **Standalone pages are now in `sitemap.xml` too (2026-07-17, see above)** — the older "Page orphan findings note standalone pages aren't in the sitemap" caveat no longer applies; an orphan `Page` finding today means genuinely no internal link points to it, not "and also not in the sitemap".
...
- The JSON-LD `@yield('json-ld')` slot is populated on every public template: `Article` schema on `blog-post.blade.php`/`tr/blog-post.blade.php`, `Person` schema on `about.blade.php`/`tr/about.blade.php`, an `Organization`+`Person` graph on `home.blade.php`/`tr/home.blade.php` — the home page is **not** missing structured data (an earlier version of this file incorrectly claimed it was; verify claims like this against the actual Blade file before repeating them) — `WebPage`/`FAQPage` on `page.blade.php`/`tr/page.blade.php` (since 2026-07-17), and `CollectionPage`/`ItemList` on `blog.blade.php`/`tr/blog.blade.php` (since 2026-07-18, closing the last real gap — see Section 8's SEO Center note and the "Missing Schema" category, which now always reports zero findings).
...
**Google Tag Manager (which hosts GA4) and Microsoft Clarity are live (added 2026-07-17), gated behind a cookie-consent banner — this is real, shipped behavior, not a plan.** Google Search Console is also connected, but separately, via a DNS TXT record (Cloudflare) — it needs no code in this repo at all, since Search Console verification and data collection happen entirely outside the site's HTTP responses.

- **GA4 is configured inside the GTM container, not directly in this codebase — a deliberate architecture change from this feature's first version.** It originally loaded GA4 directly via `gtag.js`; the same day, the user provided a real GTM container ID and, via `AskUserQuestion`, explicitly chose to move GA4 *into* GTM (as a "GA4 Configuration" tag, set up in the GTM web UI at `tagmanager.google.com` — outside this repo) rather than run both side by side (which would double-count pageviews). **Do not add a direct GA4 `gtag.js` snippet back** — GA4 configuration going forward always happens in the GTM panel, never in Blade/`config/services.php`. Any *future* tag (Meta Pixel, a conversion pixel, etc.) should likewise be added inside GTM's UI when possible, rather than as a new direct snippet in this codebase — that's the entire point of adopting GTM as the single injection point.
- **Configuration**: two env vars, `GOOGLE_TAG_MANAGER_ID` (a GTM container ID, `GTM-XXXXXXX`) and `MICROSOFT_CLARITY_ID` (a Clarity project ID), read via `config('services.google_tag_manager.id')` / `config('services.microsoft_clarity.id')` (`config/services.php`). Both are public identifiers embedded in page source by design — not secrets — but are still environment-driven (not hardcoded in Blade) so staging/local installs don't fire the production container by default. **Leaving both blank disables the feature completely**: no banner renders, no tracking script (not even the inert function definitions) is sent to the browser at all — the entire consent/tracking `<script>` block in both layouts is wrapped in `@if($gtmId || $clarityId)`, not just the banner's HTML.
- **Cookie-consent banner (KVKK/GDPR) — the explicit, confirmed requirement: no tracking script loads before a visitor accepts.** `layouts/master.blade.php` (English) and `layouts/master-tr.blade.php` (Turkish) each render a fixed-bottom banner (`#cookieConsent`, `hidden` by default in the HTML to avoid a flash-of-banner) with "Accept"/"Decline" buttons (Turkish: "Kabul Et"/"Reddet"), plus a "Learn more" link to `/privacy-policy` (`/tr/privacy-policy` on the Turkish layout). A small vanilla-JS IIFE (same "plain `<script>` at the bottom of body" convention every other piece of site-wide JS in these two files already uses — no new pattern) checks `localStorage['twe_cookie_consent']` on load:
  - `'accepted'` (from a previous visit, or set moments ago by clicking Accept) → both trackers load immediately, banner stays hidden.
  - `'declined'` → banner stays hidden, trackers never load, and it is **not** shown again on future visits (an intentional UX choice — repeatedly nagging a visitor who already declined would be worse than the KVKK/GDPR compliance gain).
  - No decision yet → banner is un-hidden (`banner.hidden = false`).
  - Clicking **Accept** sets `localStorage` to `'accepted'`, hides the banner, and loads both configured trackers (GTM via the official `gtm.js` loader snippet, minus its `<noscript>` companion — see below; Clarity via Microsoft's own official inline-snippet pattern, copied verbatim). Clicking **Decline** sets `localStorage` to `'declined'` and hides the banner — nothing is ever loaded.
  - Both loader functions are idempotent (`window.__gtmLoaded`/`window.__clarityLoaded` guards) so a returning visitor with `'accepted'` already stored never double-loads either script.
- **The GTM `<noscript><iframe>` companion tag is deliberately omitted.** Google's own install instructions say to also place a `<noscript>` iframe "as high as possible" in `<body>`, for visitors with JavaScript disabled. This project doesn't include it: a no-JS visitor can never see or interact with the consent banner (the banner's own visibility and both Accept/Decline handlers are JS-driven), so there is no way for them to grant consent — firing the noscript iframe unconditionally for that visitor would silently defeat the entire point of the consent gate. This is a deliberate privacy-first trade-off, not an oversight.
- **Search Console needs no template code.** Verified via a DNS TXT record added directly at the DNS provider (Cloudflare) for the "Domain" property type — this covers the entire domain (`trainwithehsan.com` and every subdomain/path, including `/tr/...`) automatically, the same as GTM/Clarity each need only one container/project for the whole bilingual site (no separate EN/TR setup for any of the three).
- **Core Web Vitals**: both scripts load `async` (GTM's own loader; Clarity's official snippet already sets `t.async=1`), and neither is even requested until the visitor clicks Accept (or, for a returning consenting visitor, after `DOMContentLoaded` via the same bottom-of-body script — never render-blocking, never in `<head>`). This preserves the "no heavy blocking JS" posture from Core Web Vitals Rules below.
- Rules going forward:
  - **Never remove or modify the cookie-consent gate** — GTM/Clarity must never be wired to load unconditionally on page load; that would defeat the explicit KVKK/GDPR requirement this was built for.
  - **Never add a direct GA4 `gtag.js` snippet back into this codebase** — GA4 lives inside the GTM container now, by explicit user decision; adding it back directly would double-count every pageview against the GTM-hosted GA4 tag.
  - **Never add the GTM `<noscript>` iframe** — see above; it would fire unconditionally for a no-JS visitor who has no way to consent.
  - **Never remove or modify existing tracking without explicit approval** — treat the banner markup, the loader JS, and the `config/services.php` entries as protected, the same as any other "must never be changed" category in this file.
  - Any **new tag** (Meta Pixel, conversion pixels, etc.) should be added inside the GTM container's own UI whenever possible, not as a new direct snippet in this codebase — that is the reason GTM was adopted. If a tool genuinely can't be added via GTM, wire it through the same consent-gated pattern here (add its ID to `config/services.php`, load it only inside `loadTrackers()`, guarded by the same `@if`) — never a second, differently-gated tracking mechanism.
  - `tests/Feature/AnalyticsTrackingTest.php` covers: nothing renders when both env vars are blank (no banner element, no `googletagmanager.com`/`clarity.ms` reference anywhere in the response, including inside otherwise-inert JS), the banner renders `hidden` by default when at least one is configured (either GTM alone or Clarity alone is sufficient to render it), the correct IDs are embedded, the GTM `<noscript>` iframe is genuinely absent, and the EN/TR banner text/links differ correctly per locale.
...
- Images (hero, about-section, footer background, article featured images) are referenced via `<img>`/CSS `background-image`. Article/Page featured images now prefer their WebP derivative (see Image Optimization Rules — `optimized_image_url`, falling back to the original) — a real, shipped LCP/bandwidth win — but there is still no responsive `srcset` (same image size served regardless of viewport), no explicit `width`/`height` attributes enforced as a rule, and no lazy-loading attribute convention. Any new image markup should set explicit `width`/`height` (or `aspect-ratio` in CSS) to prevent CLS, and use `loading="lazy"` for below-the-fold images — the hero/first-viewport images must NOT be lazy-loaded (that would hurt LCP).
...
- **The homepage's Instagram Showcase (see Section 6) is this site's first third-party embed, and it is fully gated behind lazy-loading — a deliberate CWV-protective design, not an afterthought.** `home.blade.php`/`tr/home.blade.php`'s `IntersectionObserver` only starts fetching `instagram.com/embed.js` once the section scrolls within `200px` of the viewport, so it never competes with above-the-fold LCP resources on initial load, and never loads at all if the admin leaves the showcase disabled or the URL blank. A `min-height:640px` reserved box (portrait ~9:16, `max-width:360px`) on `.insta-embed-wrap`/`.insta-embed-placeholder` prevents the empty-to-loaded jump that would otherwise cause CLS — this is a practical minimum-height reservation, not a strict `aspect-ratio`, since Instagram's official embed resizes to its own content and no fixed ratio fits both Posts and Reels; the card is still allowed to grow taller once real content loads. Do not switch this to eager-loading or remove the `IntersectionObserver` gate as a "simplification" — see Things That Must Never Be Changed.
...
- **Every admin media field is DAM-managed now (via the unified Media Picker, Section 31).** `ArticleForm`/`PageForm`'s `image_path`, all of `HomepageSettings`/`AboutPageSettings`/`FooterSettings`'s image and video-file fields (including repeater rows — members photo/video, About certificates & gallery), and the RichEditor's inline insert all go through `MediaPickerInput` → the picker → `MediaProcessor`, so every admin-chosen/uploaded file is a real `Media` row (WebP/thumbnail/responsive, ALT-editable, usage-tracked, delete-guarded). (`MenuSettings` has no media fields.) The earlier state where only `ArticleForm`/`PageForm` were DAM-managed is superseded — do not re-add a `FileUpload`-based media field; use `MediaPickerInput`.
...
- Static design images (hero, about section, footer background, logos, and — since 2026-07-17 — the Courses/Articles/Member-Results/Instagram-Showcase section backgrounds under `public/images/homepage/`, see Section 6) are still committed directly under `public/` and referenced by fixed path — unaffected by any of the above, and out of scope for the DAM (they're not admin-uploaded content).
...
- **Public templates now prefer the WebP derivative, with the original as an automatic fallback — explicitly requested and scoped by the user (2026-07-17), not a silent redesign.** `Article`/`Page::getOptimizedImageUrlAttribute()` (a plain `Media::forRecord($this)?->webp_url` lookup, reusing the pre-existing accessor/helper — no new query mechanism) is consumed by every public `background-image:url(...)` usage of a featured image: `home.blade.php`/`tr/home.blade.php` (latest-articles card), `blog.blade.php`/`tr/blog.blade.php` (popular + article-list thumbnails), `blog-post.blade.php`/`tr/blog-post.blade.php` (hero image, related-articles thumbnails, sidebar thumbnails), and `page.blade.php`/`tr/page.blade.php` (page hero). Each site is `{{ $record->optimized_image_url ?? asset('storage/'.$record->image_path) }}` — a record with no `Media` row (pre-DAM image, or any file `MediaProcessor` couldn't process, e.g. SVG) transparently keeps rendering its original file, exactly as before this change; nothing about the CSS, markup structure, or visual result (`background-size:cover` cropping etc.) was touched, only the URL source. **`og:image` deliberately still serves the original file, never WebP** (`page.blade.php`/`tr/page.blade.php`'s `@section('og_image', ...)`) — WebP support in social-media link-preview crawlers (Facebook, older Twitter/X card parsers, etc.) is unreliable, so this one meta tag is a permanent exception, not an oversight. **Full responsive `srcset`/`<picture>` delivery (serving a smaller variant on mobile) was deliberately not built in this pass** — every touched image is still a CSS `background-image`, which has no native responsive-by-viewport-width mechanism without either restructuring to real `<img>` tags or adding per-breakpoint media queries to the hand-rolled, pixel-matched CSS (Brand Identity); `Media::responsive_urls` remains generated on every upload and ready to use, but wiring it in is a separate, larger markup decision — see Future Development Guidelines.
...
- **`alt_text`/`caption`/`description` are the fields whose Apply/Restore target is a *different* model — generalized as `ActionRegistry::MEDIA_BACKED_FIELDS`, see Section 30.** They live on the DAM's `Media` row, keyed by matching `Media.disk_path` to the record's `image_path` — via `Media::forRecord(Model $record): ?Media`, a static helper on the `Media` model itself (extracted here specifically because both `AiAssistantPanel` and `App\Jobs\ProcessAiChatMessage` need the same three-line lookup — the second real caller is what justified pulling it out of `AiAssistantPanel::mediaForRecord()`, which now just delegates to it). `GenerationApplier::apply()`/`::restore()` check `in_array($generation->field, ActionRegistry::MEDIA_BACKED_FIELDS, true)` to read/write `Media` instead of the Article/Page record — originally this was a single hardcoded `field === 'alt_text'` check (this section's original text); `caption`/`description` were added to both the constant and to `Media`'s own schema by the AI Image Pipeline (Section 30), which is also what made `caption` a real, appliable field instead of a no-storage suggestion.
...
- **Fields with genuinely no CMS home are detected and reported, never silently dropped or stored
  ...
  prompt pasted as part of an imported article's JSON/Markdown — a different thing from
  `Article.hero_image_prompt`, the real, admin-editable column the AI Image Pipeline added later,
  Section 30; a pasted `featured_image_prompt` value is still never written anywhere by the
  importer) all land in the mapping report's `skipped` bucket with a plain-language reason —
  `schema` stayed suggestion-only when this was written; `caption` later became a real, appliable,
  `Media`-backed field via the Image Pipeline (Section 30), so that parenthetical no longer applies
  to `caption` specifically, only to `schema` and to this importer's own always-skipped bucket.
...
- **Small, additive extensions were needed to make reuse possible, not a rebuild of the reused
  ...
  (the `Media`-branch of `missingAlt()`, `allLinkSources()`'s per-source `meta`, and
  `checkExternalLinks()`'s finding array) gained one new
  key, `'id'` — the underlying record's own id (or `null` where there genuinely isn't one, e.g. the
  blog-index entries `missingSchema()` used to return before it started always returning `[]`
  in 2026-07-18) — so `AgentAuditService` can route a finding back to a fixable record without
...
## 30. AI Image Pipeline

One-click "Generate Hero Image" for an Article/Page — a provider-agnostic image-generation layer
that mirrors the text-provider architecture (Section 24) exactly, and a job that generates the
image, saves it through the **unmodified** Media Library pipeline (Section 21), sets it as the
featured image, and auto-fills ALT text/caption/description plus any still-blank SEO fields through
the **unmodified** `ActionRegistry`/`ContentAssistantService`/`AiGeneration`/`GenerationApplier`
pipeline (Section 23). Two binding decisions were confirmed with the user via `AskUserQuestion`
before any code was written — see Important Project Decisions for the full quotes: (1) the image
provider layer must be **architecturally identical** to the text-provider layer — admin-configurable
per-provider enable/API-key/priority, a selectable default, and automatic failover, never a
hardcoded vendor; (2) generate **one** high-quality Hero image today, but store four **separate**,
independently editable prompts (Hero/Thumbnail/OG/Social) so a future version can generate the other
three without a database redesign — "even if only the Hero image is generated today."

- **`ImageProvider` — a third per-vendor contract, parallel to `AiProvider`/`EmbeddingProvider`, OpenAI/Gemini only.**
  `App\Services\AiAssistant\Contracts\ImageProvider` (`generateImage(string $prompt, array $options =
  []): array{bytes: string, mime_type: string, revised_prompt: ?string}`, `lastImageUsage(): ?array`)
  is implemented by `OpenAiProvider` (`POST /v1/images/generations` — handles both the `b64_json` and
  `url` response shapes, since `gpt-image-1` always returns base64 while `dall-e-3`/`dall-e-2` default
  to a URL that must be fetched) and `GeminiProvider` (`POST /models/{model}:predict` — the Imagen
  endpoint on the same Generative Language API `respond()`/`embed()` already use, base64 image
  returned directly in `predictions.0.bytesBase64Encoded`, no URL to fetch). Exactly like
  `EMBEDDING_CAPABLE_SLUGS` (Section 27), `AiProviderConfig::IMAGE_GENERATION_CAPABLE_SLUGS =
  ['openai', 'gemini']` is the single source of truth for which rows can be picked as the image
  provider — Anthropic/Grok/DeepSeek have no image-generation API and do not implement the interface.
  `AiProviderConfig::image_model` (nullable, meaningful only on the OpenAI/Gemini rows) and
  `is_usable_for_image_generation` (usable AND image-capable slug AND `image_model` set) follow the
  exact same shape as `embedding_model`/`is_usable_for_embeddings`.
- **`ProviderManager::generateImage()` replicates `respond()`'s full multi-candidate failover — not
  a simplified version, per the explicit requirement.** This is a deliberate difference from
  `embed()` (Section 27), which has **no** failover because a failed embedding call always has the
  keyword-search fallback to degrade to; there is no equivalent fallback for "the image didn't
  generate," so `resolveImageCandidates()` builds a real candidate list (primary
  `default_image_provider_config_id`, plus `fallback_image_provider_config_id` appended only when
  `image_failover_enabled` is on and the fallback is itself usable and different from the primary) and
  `attemptImage()` retries each candidate up to twice (300ms backoff) exactly like `attempt()` does for
  text — same shape, different array payload. `resolveImageProvider(): ?AiProviderConfig` mirrors
  `resolveEmbeddingProvider()` so the UI (the sidebar's "Generate Hero Image" button, `AiActionRouting`'s
  Image Generation section) can check "is anything configured at all" without touching the private
  candidate-resolution method. **No `.env`-only fallback exists for images** — same reasoning as
  embeddings (Section 27): image generation is a brand-new capability with no pre-existing installs to
  stay compatible with, so "nothing configured" simply means the button is disabled, not a hidden
  default to wire up. Every attempt — success or final failure — writes one `ai_usage_logs` row
  (`action_key: 'image_generation'`), reusing the same table every other AI call already uses;
  `estimated_cost_usd` is always `null` for image rows — `ai_provider_models`' pricing catalog is
  token-based (input/output $ per million tokens), not applicable to a flat per-image price, so a null
  is honest here rather than a fabricated per-token estimate (same "null over a guess" posture as
  `SuggestionEngine`/`Media::warnings()` elsewhere in this codebase).
- **`App\Jobs\GenerateHeroImage` — "generate" IS "the real write," the same posture as
  `TranslateArticleDraft` (Section 23), not the two-step preview-then-apply flow most `ActionRegistry`
  fields use.** There is no separate "Apply" click for a generated hero image — by the time the job
  finishes, the image is already a real `Media` row and already the record's `image_path`, because a
  freshly generated image has no "previous value" worth previewing against (unlike a rewritten SEO
  title). The prompt is `Article`/`Page.hero_image_prompt` when the admin filled it in, otherwise a
  string built automatically from the record's title (+ category/excerpt for Articles) — this is the
  literal mechanism behind "even if only the Hero image is generated today, nothing requires the admin
  to write a prompt first." The generated bytes are wrapped in a real `Illuminate\Http\UploadedFile`
  (`new UploadedFile($tmpPath, $filename, $mimeType, null, true)`, the exact `test: true`-flag pattern
  `ArticleImportService::downloadImage()` already established for a programmatically-created file —
  Section 28) and handed to the **unmodified** `App\Services\Media\MediaProcessor::store()`, which is
  what generates the WebP + thumbnail + responsive variants and registers a real, DAM-tracked `Media`
  row — **zero changes to `MediaProcessor` were needed**, fully satisfying "reuse existing media
  management, do NOT rebuild." Every AI-generated image lands in a dedicated `MediaFolder` named "AI
  Generated" (`firstOrCreate(['name' => 'AI Generated', 'parent_id' => null])`), auto-created on first
  use — this keeps AI-sourced assets visually distinguishable in the Media Library grid without a new
  DAM concept.
- **After the image is saved, a small, fixed set of text fields are auto-generated through the
  existing pipeline — never a second write path.** `GenerateHeroImage::autoGenerateMetadata()` splits
  into `ALWAYS_FIELDS = ['alt_text', 'caption', 'description']` (always regenerated, since the image
  is brand new — there is nothing worth preserving) and `BLANK_ONLY_FIELDS = ['seo_title',
  'meta_description', 'og_title', 'og_description']` (generated **only** when the field is currently
  blank — existing manually-written SEO copy is never silently overwritten by an image-generation
  side effect). Each field goes through `ContentAssistantService::generate()` → a real `AiGeneration`
  row (`status: 'completed'`, since the result is already known) → `GenerationApplier::apply()` —
  exactly the same three-step pipeline every other field's Apply button uses, so every
  auto-filled value shows up in the record's History tab like any manually-triggered generation, not a
  parallel, invisible write. A failure generating one auxiliary field (e.g. a rate limit on
  `seo_title`) is caught and `report()`-ed per-field — it must never make an already-successfully-saved
  image read as "failed."
- **`MEDIA_BACKED_FIELDS` generalizes what used to be a hardcoded `'alt_text'` special case —
  `caption` and the new `description` field are real now, not suggestion-only.** Before this feature,
  `caption` was `appliable: false` with no storage anywhere (Section 23's original text, now
  superseded) and `alt_text` was the only field whose Apply/Restore target was `Media`, not the
  Article/Page record — hardcoded as `$generation->field === 'alt_text'` in three separate places
  (`GenerationApplier`, `AiAssistantPanel`, `ContentAssistantService::imagesFor()`). This feature adds
  real `Media.caption`/`Media.description` columns, flips `caption` to appliable, adds a new
  `description` field, and replaces every one of those hardcoded checks with
  `ActionRegistry::MEDIA_BACKED_FIELDS = ['alt_text', 'caption', 'description']` — the exact same
  "generalize a growing special case into a registry-driven check" pattern `MEDIA_BACKED_FIELDS`'s own
  name signals. All three fields are still generated/applied/restored through the ordinary sidebar UI
  too (not just by `GenerateHeroImage`) — an admin can regenerate just the ALT text or caption for an
  existing featured image at any time, exactly like before.
- **"One image, four prompts" — the confirmed future-ready design, with zero speculative code.**
  `hero_image_prompt`/`thumbnail_image_prompt`/`og_image_prompt`/`social_image_prompt` are four real,
  independent `text` columns on both `Article` and `Page` (mirroring the `seo_title`/`meta_description`/
  `og_title`/`og_description` column quartet added in Section 23 — same "real column, not a JSON blob"
  shape). Only `hero_image_prompt` is consumed by any image-generation code today; the other three are
  wired into the **existing, unmodified** `ActionRegistry`/`ContentAssistantService::generate()`/
  `GenerationApplier` text-generation pipeline (`response_shape: 'text'`, modes
  `generate`/`improve`/`rewrite`/`expand`/`shorten`) so an admin can already write, AI-draft, and edit
  all four prompts from the ordinary sidebar UI and the collapsed "AI Image Prompts" section on
  `ArticleForm`/`PageForm` — even though no `generateThumbnailImage()`/`generateOgImage()`/
  `generateSocialImage()` job exists yet. This is the literal mechanism behind "design the architecture
  so future versions can optionally generate separate OG/Social/Thumbnail images without database
  redesign or breaking existing content" — adding real generation for one of the other three prompts
  later is a new job class (a thin variant of `GenerateHeroImage`, writing to a not-yet-existing
  dedicated column instead of `image_path`) plus a UI button, not a schema change.
- **`App\Models\AiImageGeneration`** is the progress/history table for hero-image generation —
  modeled on `AiGeneration`'s shape but kept as a **separate** table rather than reusing
  `ai_generations`, because its output is binary/`Media`-shaped, not text/array-shaped, and doesn't fit
  `ContentAssistantService::parseResponse()`'s `response_shape` model at all. Columns: `content_type`/
  `content_id` (same short-string morph convention as every other polymorphic reference in this
  codebase), `prompt_field` (`hero_image_prompt` today, always — the column exists so a future
  thumbnail/OG/social job can reuse this same table instead of inventing a fourth), `prompt_used` (the
  actual resolved prompt sent to the provider — useful for "why did it generate this?"), `status`
  (`queued`/`processing`/`completed`/`failed`/`cancelled` — the same five-state shape as
  `AiGeneration`), `provider_slug`/`model` (denormalized, same reasoning as `ai_usage_logs` — a
  provider config being renamed/disabled later must never break historical display), `media_id`
  (nullable FK to the resulting `Media` row, `nullOnDelete` since deleting the Media row later must
  not cascade-delete the history entry), `error`, `user_id`. `isCancellable()`/`scopeForRecord()`
  mirror `AiGeneration`'s methods exactly.
- **Cancellation — the same two checkpoints, the same honest "can't kill an in-flight HTTP call"
  ceiling, established in Section 23 for `RunAiContentGeneration`/`TranslateArticleDraft`.**
  `GenerateHeroImage` checks once before starting (skip entirely if already `cancelled` — catches a
  cancel landing between dispatch and worker pickup) and once more via a `fresh()` re-read
  immediately after the image-generation API call returns, before anything is written to the Media
  Library or the record — so a cancel that happens mid-API-call never leaves an orphaned `Media` row
  or a half-applied featured image behind, the same invariant `TranslateArticleDraft`'s pre-creation
  checkpoint already guarantees for a translated draft.
- **Filament UI, additive to the existing screens established in Sections 6/24 — no new nav items.**
  `AiProviderConfigForm` gained an `image_model` `TextInput`, visible only on the OpenAI/Gemini rows
  (`IMAGE_GENERATION_CAPABLE_SLUGS`), directly beneath the existing `embedding_model` field.
  `AiActionRouting` gained an "Image Generation" `Section` (default image provider / failover toggle /
  fallback image provider — the exact same three-field shape as "Global defaults," via a new
  `imageProviderSelect()` helper that combines `providerSelect()`'s toggle-based failover UI with
  `embeddingProviderSelect()`'s "(not ready — set API key + image model)" labeling), writing to three
  new `ai_provider_settings` columns (`default_image_provider_config_id`/`image_failover_enabled`/
  `fallback_image_provider_config_id`) that mirror the existing default/failover/fallback trio 1:1.
  The `Media` section of the field-override grid (same per-field provider-override UI every
  `ActionRegistry` field already gets) picked up `description`/`hero_image_prompt`/
  `thumbnail_image_prompt`/`og_image_prompt`/`social_image_prompt` automatically, since that grid is
  driven entirely by `ActionRegistry::all()` — no new UI code was needed for those five fields beyond
  listing their keys in `AiActionRouting::SECTIONS['Media']`. `ArticleForm`/`PageForm` each gained a
  collapsed "AI Image Prompts" section with four plain `Textarea`s — same collapsed-by-default,
  optional-field posture as the existing "SEO & social preview" section.
- **The embedded AI Assistant sidebar (Section 23) gained a "Hero Image" card**, rendered above the
  Translate card: a "✨ Generate Hero Image" button (only shown when
  `ProviderManager::resolveImageProvider()` returns something, otherwise a plain "no provider
  configured" message linking to AI Routing) and the latest 5 `AiImageGeneration` rows for this record
  with inline status/thumbnail/cancel, following the exact same queued/processing/completed/failed
  rendering convention the rest of the sidebar already uses. `AiAssistantPanel::generateHeroImage()`/
  `cancelImageGeneration()`/`getCanGenerateImagesProperty()`/`getHeroImageGenerationsProperty()` are
  thin wrappers — create the row, dispatch the job, or flip a status — following the exact
  `generateField()`/`cancelGeneration()` pattern already established for text fields.
  `getIsPollingProperty()` was extended to also poll while any `AiImageGeneration` for this record is
  `queued`/`processing`, so the sidebar keeps auto-refreshing during image generation without a second
  polling mechanism.
- **Testing**: `tests/Feature/AiImagePipelineTest.php` covers `AiProviderConfig::IMAGE_GENERATION_CAPABLE_SLUGS`/
  `is_usable_for_image_generation`, `OpenAiProvider`/`GeminiProvider::generateImage()` against
  `Http::fake()` (both OpenAI response shapes, Gemini's `predictions` shape),
  `ProviderManager::resolveImageProvider()`/`generateImage()` (no-provider throw, usage logging,
  failover to a working fallback after the primary fails, all-candidates-fail throw, a
  selected-but-not-image-capable provider never being returned), `GenerateHeroImage`'s full
  end-to-end happy path (Media Library save, featured-image set, `ALWAYS_FIELDS` always
  regenerated, `BLANK_ONLY_FIELDS` only filling genuinely blank fields, real `AiGeneration` history
  rows), automatic-vs-custom prompt resolution, both cancellation checkpoints, the
  record-no-longer-exists guard, `AiImageGeneration`'s `isCancellable()`/`scopeForRecord()`,
  `AiAssistantPanel`'s new methods, and the Filament UI additions (`image_model` field visibility,
  `AiActionRouting`'s Image Generation section save/clear-on-disable, and the four prompt fields
  rendering on both `ArticleForm` and `PageForm`). The full pre-existing suite (474 other tests)
  passes unchanged, confirming this feature is additive — nothing in Sections 21/23/24 was rebuilt.

## 31. Unified Media Picker

Every media/file selector across the entire CMS opens **one** reusable Media Picker — a
professional DAM modal (grid/list, folders, filters, in-picker upload, preview panel). There
are no searchable-dropdown media selectors or per-field "choose from library" helper buttons
left anywhere. Built in five reviewed phases (PRs #83–#87); this is purely a **widget/UX**
change — every field still stores exactly what it stored before, so it is fully
backward-compatible with zero data migration.

- **`App\Filament\Concerns\InteractsWithMediaLibrary`** is the single source of truth for all
  Media Library mechanics — folder navigation/CRUD, drag-and-drop/multi upload (→
  `MediaProcessor`, WebP/thumbnail/responsive), search, type/unused/orphan/missing-ALT/large
  filters, the detail panel (metadata, usage, WebP status + savings, ALT/caption/description
  editing), replace, delete-guard, and regenerate-WebP (single + bulk). Both
  `App\Filament\Pages\MediaLibrary` (the full-page screen) and `App\Livewire\MediaPicker` (the
  modal) `use` this trait, so there is exactly **one** implementation, never two parallel
  copies. Extension points on the trait: `searchableColumns()` (the page searches filename
  only; the picker overrides it to also match ALT/caption/description/mime) and
  `$uploadDirectory` (default `media/library`; the picker sets it per-field so a new upload
  lands in that field's folder and orphan tracking stays intact).
- **`App\Livewire\MediaPicker`** is the modal, mounted **once globally** via an
  `AdminPanelProvider` `PanelsRenderHook::BODY_END` render hook (`@livewire('media-picker')`) —
  so it exists on every panel page but costs almost nothing until opened (only its root + a
  window-event listener render while closed; the heavy grid/query is inside an `@if($isOpen)`).
  It adds grid/list view toggle, the full top-filter set (All/Images/Videos/Documents/Audio/
  Archives/Other/Unused, with Favorites stubbed as a disabled "coming soon" chip — future-ready
  with no schema change), single-click-selects / double-click-inserts, and the open/return
  contract below. `MediaPicker::icon()` maps non-image types to a file-type emoji for the grid.
- **`App\Filament\Forms\Components\MediaPickerInput`** is the **only** field every media
  selector uses (Article/Page featured image, all Homepage/About/Footer image fields, repeater
  rows — members photo/video, About certificates & gallery — and video-file fields). It stores
  a plain `disk_path` **string** — byte-identical to what the old `FileUpload` +
  `saveUploadedFileUsing(MediaProcessor::store)` stored — so every existing reader
  (`BlogController`, the public Blade templates, `SeoController`), every stored value, and the
  settings pages' `mount()`/`save()` (which already normalize a string) keep working untouched.
  Modifiers: `->onlyImages()` (hard-locks the picker to images and hides the type chips — for
  Featured/Hero/Banner/etc.), `->uploadDirectory(...)` (where a new in-picker upload lands),
  and `->initialType('video')` (a **soft** default view — e.g. video-file fields open on the
  Videos filter but still allow any type, honoring "generic file fields allow every media type";
  `onlyImages` always overrides it). Because it is one field class, **all future media fields
  get the unified picker for free** — just use `MediaPickerInput::make(...)`.
- **Event contract (media-type-agnostic on purpose).** A field opens the picker with a window
  `CustomEvent` `open-media-picker` carrying `{target, onlyImages, uploadDirectory, initialType}`
  (`target` = the field's resolved `statePath`); the picker's root Alpine listener calls
  `openFor(...)`. On selection the picker fires a window `CustomEvent` `media-picker-selected`
  with `{target, disk_path, url, type, mime_type, original_name}` (dispatched via `$this->js(...)`
  so it's a guaranteed browser event, independent of Livewire↔Alpine event nuances); only the
  field whose `statePath` equals `target` sets its state (`$wire.set(target, disk_path)`). This
  exact-`statePath` targeting is what makes **repeater rows** work with no shared-state bleed —
  each row's field renders with its own `…<uuid>.field` path. The field-blade Alpine uses the
  standard `@js(...)`-inside-`x-data` pattern (an inline nested `x-on:click` expression was tried
  first and threw an "Invalid or unexpected token" parse error — don't go back to it). Save
  buttons in the picker call Livewire via `x-on:click="$wire.saveAltText($refs.mpAlt.value)"`,
  **not** `wire:click="saveAltText($refs.mpAlt.value)"` — the latter makes Livewire's directive
  scanner eagerly evaluate `$refs.mpAlt` (undefined at scan time) and throw.
- **RichEditor.** `App\Filament\RichContent\MediaLibraryRichContentPlugin`'s toolbar "Media
  Library" button opens a small Insert modal whose only media control is the same
  `MediaPickerInput` (the unified picker opens **over** the action modal — verified to be fully
  interactive, no focus-trap problem), plus an ALT field. Insertion is type-aware via a pure,
  testable `insertContentFor(Media, ?alt)` that reuses the sanctioned `runCommands` +
  `editorSelection` path: **image** → an `<img src alt>` node (original-file URL so
  `MediaUsageScanner` keeps tracking it; survives `Str::sanitizeHtml` #73), **document/zip/
  other** → a download link (`<a href>`, the one non-image tag the current sanitizer keeps).
  Video/audio players and YouTube/Vimeo/Instagram/TikTok embeds are a **deliberate future
  one-branch addition** to `insertContentFor()` — they need the sanitizer allowlist opened up,
  which is the separate Video-SEO / External-Media phase; the picker itself needs no change for
  them.
- **Removed:** `App\Filament\Support\MediaLibraryUploads` (its `callback()`/`pickFromLibraryAction()`
  /`altHintAction()` — the old `saveUploadedFileUsing` wrapper, the searchable-`Select` "pick from
  library" modal, and the inline ALT hint action) is **deleted** — nothing references it. ALT is
  now edited inside the picker's preview panel. Do not reintroduce a `saveUploadedFileUsing` +
  `FileUpload` media field or a searchable-`Select` media picker; use `MediaPickerInput`.
- **Knowledge Base attachments are intentionally NOT on this picker.** They remain their own RAG
  document-ingestion pipeline (`KnowledgeEntryAttachment`, extract→chunk→embed — Section 27),
  not a DAM media field. The architecture is designed so they *could* adopt the picker UI later
  without changing their storage/indexing model, but that was explicitly out of scope.
- **Testing**: `tests/Feature/MediaPickerTest.php` (open/close, only-images lock, `initialType`
  soft-default-vs-override, doc/audio/archive type mapping, extended search, choose-and-return,
  image-only guard, field config, Article/Page featured-image round-trip, edit-page render);
  `tests/Feature/CmsUploadWiringTest.php` (a `MediaPickerInput` value round-tripping through the
  Homepage/About/Footer `save()` into `SiteSetting`, including a repeater-embedded members photo/
  video_file); `tests/Feature/MediaMetadataUxTest.php` (ALT/caption/description edited from the
  picker); `tests/Feature/RichEditorMediaPickerTest.php` (`insertContentFor` image node + ALT
  fallback, document download link + sanitize survival, inline-image usage tracking, form-still-
  mounts). `tests/Feature/MediaLibraryTest.php` (unchanged) still covers the shared trait's
  mechanics. Repeater and modal-over-modal interactions were also browser-verified with Playwright.
...
Video SEO is covered by `tests/Feature/VideoSchemaTest.php` (homepage `VideoObject`), `tests/Feature/EmbedRendererTest.php`/`HomepageVideoEmbedTest.php` (facade detection/rendering), and — since 2026-07-18 — `tests/Feature/VideoSeoTest.php` (C1/C2: in-body Article/Page `VideoObject` for YouTube/Vimeo/self-hosted, invalid-skipped, multiple-per-page, inline-links-ignored, no-duplicate, the homepage-output-unchanged regression, HTTP-rendered valid JSON-LD with no eager `<iframe>`, and the `<video:video>` sitemap incl. well-formed-XML/namespace assertions).
...
## 34. Important Project Decisions
...
- **The AI Image Pipeline's provider layer must stay architecturally identical to the text-provider layer — a confirmed, explicit requirement, not a default design choice.** The user's own words, via `AskUserQuestion`: "Do not hardcode image providers. Create a provider abstraction identical to the text AI provider architecture. Allow administrators to: Enable/disable providers, Set provider priority, Select default image provider, Configure API keys independently, Switch providers without changing application code. If one provider fails, automatically fall back to the next enabled provider." This is why `ProviderManager::generateImage()` replicates `respond()`'s full multi-candidate failover instead of reusing `embed()`'s deliberately-simpler no-failover design (Section 27) — image generation has no equivalent fallback path the way retrieval has a keyword shortlist, so it genuinely needs the same resilience text generation gets. Do not simplify `generateImage()`'s failover away as a "cleanup."
- **"One image today, four prompts always" is a confirmed, explicit scope decision, not an incomplete feature.** The user's own words: "Generate one high-quality Hero image only. ... Store editable prompts separately for: Hero, Thumbnail, Open Graph, Social. Even if only the Hero image is generated today. Design the architecture so that future versions can optionally generate separate OG, Social or Thumbnail images without database redesign or breaking existing content." `thumbnail_image_prompt`/`og_image_prompt`/`social_image_prompt` having no dedicated image-generation job yet is intentional, confirmed scope — not a gap to silently "finish" as part of an unrelated task; building real generation for one of them is a legitimate future feature (see Future Development Guidelines) that needs its own sign-off, same as any other new capability.
- **GTM/Microsoft Clarity's cookie-consent gate was an explicit user choice, confirmed via `AskUserQuestion`, not a default this project happened to build.** The user was asked whether a consent banner should exist before tracking fires (given the site targets Turkish/KVKK and EU/GDPR visitors) and confirmed "yes, add it" — this is why the entire tracking `<script>` block, not just the banner's visibility, is gated behind `@if($gtmId || $clarityId)` and a `localStorage`-backed accept/decline decision (see Analytics & Tracking). Do not simplify this into an always-on snippet as a "cleanup."
- **GA4 moved from a direct `gtag.js` snippet into being configured inside the GTM container — also an explicit user choice, made the same day, confirmed via `AskUserQuestion`.** The user provided a real GTM container ID and chose "move GA4 into GTM" over "add GTM as a third, separate script" specifically to avoid double-counting pageviews (one from direct `gtag.js`, one from a future GTM-configured GA4 tag). Do not re-add a direct GA4 snippet alongside GTM — GA4 configuration lives in the GTM web UI only, never in this codebase, going forward.
- **Contact/FAQ/legal pages (2026-07-17) were built entirely inside the existing standalone Pages system — an explicit instruction, not a default choice.** The request was specific: reuse the `Page` model/`PageResource`/`/{slug}` routing, do not create a "Legal Pages", "Contact Manager", or "FAQ Manager" CMS module. Concretely: `Contact`, `FAQ`, `Privacy Policy`, `Terms & Conditions`, `Cookie Policy`, and `Disclaimer` are six ordinary `Page` rows (one `en` + one `tr` each, `translation_of`-linked), seeded via `2026_07_17_000011_seed_contact_faq_legal_pages.php` (same "seed migration inserts real, admin-editable rows" convention as `ai_provider_configs`/`brand_memory_sections`/`workflow_stages`) — every one of them is editable from the same `/admin/pages` screen as any other page, with no special-cased admin UI. Do not split these back out into a dedicated resource/page/table.
- **`page.blade.php`/`tr/page.blade.php` special-case exactly one thing by `slug`: the Contact page's contact-info block and form (`@if($page->slug === 'contact')`).** This was a deliberate, minimal exception to the "one generic Page template" rule — a real working contact form needs a POST target/anti-spam/mail-sending, which isn't expressible as RichEditor `body` HTML the way the other five pages' content is. Do not special-case any *other* slug in this template; if a future page needs interactive behavior beyond static HTML, that's a new, explicitly-scoped decision (matching how Home/About/Blog already have their own dedicated Blade views + controllers for exactly this reason — see Section 4).
- **The Contact form reuses the newsletter form's exact anti-spam architecture (honeypot field `website` + an encrypted render-timestamp field + a named rate limiter) rather than inventing a new one** — `App\Http\Controllers\ContactController::submit()` mirrors `NewsletterController::subscribe()` line-for-line in structure. It sends one email (`App\Mail\ContactMessageMail`, reply-to the visitor) to whatever `footer.en.contact_email` (`SiteSetting`, edited in the existing Footer Settings page) is currently set to — if that's blank, the form returns a clear "not available right now, email us directly" error rather than silently failing or sending to nowhere. No new database table was added for contact submissions (direct email only, matching "no new abstraction without a concrete second use case" — see Coding Standards).
- **`SeoAuditService::missingSchema()` no longer flags anything at all — `Page` (2026-07-17) and the blog index (2026-07-18) both went from "genuinely missing JSON-LD" to "genuinely has real JSON-LD," closing every gap the audit existed to surface (Section 8/9's Future Development Guidelines item 4, now fully done), not weakening the check.** `blog.blade.php`/`tr/blog.blade.php` now emit a `CollectionPage`/`ItemList` schema listing every article shown on the page — see Section 8. The method takes no arguments and is a permanent `return []` tripwire now, same posture as `missingCanonicals()`. The AI Agent dashboard's `missing_schema` category (Section 29, a thin wrapper over this same service) inherited both fixes automatically with no code change of its own.
- **The homepage Instagram Showcase (2026-07-17, see Section 6) was built as a pure extension of `HomepageSettings`, per an explicit, detailed user instruction — not a default architectural choice.** The request enumerated exactly what must NOT happen: no new Homepage resource, no new Instagram/Social Media module, no new CMS page, no rebuilding/replacing `HomepageSettings`, no renaming/deleting/moving any existing field. It also specified the embed must be Meta's **official** embed only (no third-party widget). **The same day, three follow-up requests refined it: (1) a second, independent Instagram row (referencing `ehsandibazar.com`, which shows two bands for two different Instagram pages) — implemented as `insta_showcase2_*` fields mirroring Row 1, off by default so Row 1-only configurations render unchanged; (2) a near-9:16 portrait embed frame ("تصویر اینستاگرام نزدیک ۹ در ۱۶ حالت عمودی هست") replacing the original build's more-square `550×460`; (3) removal of the now-redundant old "Instagram" section and its four orphaned image fields (`insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image`) — "if it's not depended upon, remove it" — while keeping `insta_url` (which IS depended on, as the shared Follow-button fallback) folded into Row 1's Showcase section.** This last request explicitly reverses the earlier "keep the old image fields intact" decision — the old fields were confirmed unreferenced and were deleted from `instaFields()`/`FILE_KEYS`/the form; only `insta_url` was retained. Do not collapse the two rows back into one, do not widen the embed frame back toward square, and do not re-add the old Instagram image fields — all three without a new explicit request.
- **This environment's network policy blocks outbound access to `ehsandibazar.com` (confirmed 2026-07-17: both a direct `curl` and the `WebFetch` tool receive a `403`/gateway rejection) — a real, current constraint, not a one-off fluke.** Earlier sessions this repo's history references being able to browse the reference site is describing the *user's* own browser, not this agent's network access. When a task needs the reference site's actual assets (not just its visual layout, which the user can screenshot), the correct move confirmed in practice is to ask the user to download and upload the real file themselves (see the Courses/Articles/Member-Results/Instagram-Showcase background images in Section 6) rather than guessing at a CSS/SVG approximation or silently attempting a fetch that will fail. Don't assume network access to this specific domain has changed without re-verifying.

## 35. Things That Must Never Be Changed
...
- Do not couple `ProviderManager::generateImage()`, `GenerateHeroImage`, or any future image-generation caller to a specific vendor — go through `App\Services\AiAssistant\Contracts\ImageProvider`, the same "never a concrete provider class directly" rule already enforced for text (`AiProvider`) and embeddings (`EmbeddingProvider`). Do not add a `.env`-only fallback for image generation the way `respond()` has one for chat — same reasoning as embeddings (Section 27): this is a brand-new capability with no pre-existing installs to preserve compatibility for; `resolveImageProvider()` returning `null` (→ the button simply stays disabled) when nothing is configured is correct, complete behavior, not a gap.
- Do not let `App\Jobs\GenerateHeroImage` (or any future thumbnail/OG/social image-generation job) bypass `App\Services\Media\MediaProcessor::store()` — every AI-generated image must go through the exact same WebP/thumbnail/responsive/DAM-registration pipeline every other upload path uses (see Image Optimization Rules and Section 30). Do not write image bytes to disk directly.
- Do not remove `GenerateHeroImage`'s two cancellation checkpoints (before starting, and a `fresh()` re-read right after the image-generation API call returns, before anything is saved to the Media Library or written to the record) — the same "cancel must never leave an orphaned write behind" invariant already established for `RunAiContentGeneration`/`TranslateArticleDraft` (see Section 23 and the corresponding entry above), now applied a third time here.
- Do not let `GenerateHeroImage`'s automatic ALT text/caption/description/SEO-field generation write directly to the Article/Page/Media record — each field must go through the existing `ContentAssistantService::generate()` → a real `AiGeneration` row → `GenerationApplier::apply()` pipeline (Section 30), the same "no second write path" rule already enforced for the AI Agent dashboard's one-click fixes (see the `GenerationApplier` entry above). Do not make `BLANK_ONLY_FIELDS` (`seo_title`/`meta_description`/`og_title`/`og_description`) overwrite a field that already has content — image generation must never silently clobber existing manually-written SEO copy.
- Do not remove `Media.caption`/`Media.description` from `ActionRegistry::MEDIA_BACKED_FIELDS`, and do not reintroduce a hardcoded `field === 'alt_text'` check anywhere in `GenerationApplier`/`AiAssistantPanel`/`ContentAssistantService` in place of `in_array($field, ActionRegistry::MEDIA_BACKED_FIELDS, true)` — the constant exists specifically so a future Media-backed field is a one-line addition, not a repeat of the three-places-hardcoded state this replaced (see Section 30).
- Do not remove the `?? asset('storage/'.$record->image_path)` fallback from any public template's use of `optimized_image_url` — a record whose image predates the DAM, or whose file `MediaProcessor` couldn't process (e.g. SVG), has no `Media` row and must keep rendering its original file, never a broken/missing image (see Image Optimization Rules). Do not change `page.blade.php`/`tr/page.blade.php`'s `og:image` meta tag to use the WebP derivative — social-media link-preview crawlers have unreliable WebP support, so `og:image` must keep serving the original file, permanently, not as a temporary gap.
- Do not make GTM/Microsoft Clarity load unconditionally on page load — both must stay behind the cookie-consent banner's `localStorage['twe_cookie_consent'] === 'accepted'` gate (Analytics & Tracking) — this was an explicit KVKK/GDPR compliance requirement, not a default the code happened to end up with. Do not remove the `@if($gtmId || $clarityId)` wrapper around the entire consent/tracking `<script>` block in `master.blade.php`/`master-tr.blade.php` — an unconfigured install must send zero tracking-related code to the browser, not just hide the banner.
- Do not add a direct GA4 `gtag.js` snippet back into `master.blade.php`/`master-tr.blade.php`, and do not add the GTM `<noscript><iframe>` companion tag — both were explicit, confirmed decisions (Important Project Decisions), not gaps to "complete." GA4 lives inside the GTM container (configured at tagmanager.google.com, not in this repo) to avoid double-counting; the noscript iframe is omitted because a no-JS visitor has no way to grant consent through the JS-driven banner, so firing it unconditionally would defeat the consent gate.
- Do not create a separate "Legal Pages"/"Contact Manager"/"FAQ Manager" CMS module — Contact, FAQ, and every legal page are ordinary `Page` rows managed from the existing `/admin/pages` screen, per the explicit instruction behind this feature (see Important Project Decisions). Any future standalone page (a new legal doc, a landing page, etc.) belongs in this same system, not a new one.
- Do not special-case any additional `slug` value inside `page.blade.php`/`tr/page.blade.php` beyond the existing `'contact'` check — that one exception exists because a real contact form needs server-side plumbing a RichEditor body can't express. A new page needing interactive behavior should get its own dedicated Blade view + controller (the established pattern already used for Home/About/Blog), not a second special-cased slug bolted onto the generic Page template.
- Do not remove or bypass the Contact form's honeypot/timestamp/rate-limit anti-spam trio (`App\Http\Controllers\ContactController`) — it deliberately mirrors `NewsletterController`'s proven pattern; do not build a second, differently-shaped anti-spam mechanism for a future form. Do not make the Contact form silently fail or send to a hardcoded address when `footer.en.contact_email` is blank — it must keep returning the clear `contact.not_configured` error so a misconfigured install is visibly broken, not silently losing messages.
- Do not re-introduce a hardcoded list of static slugs in `SeoController::sitemap()` for standalone pages — it loops every published `Page` record (`Page::published()->get()`) exactly like `Article`, specifically so a newly published page never needs a code change to appear in the sitemap. Do not let `SeoAuditService::missingSchema()` start flagging `Page` rows, `Article` rows, or the blog index again without first checking whether `page.blade.php`/`tr/page.blade.php` still emit `WebPage` schema, `blog-post.blade.php`/`tr/blog-post.blade.php` still emit `Article` schema, and `blog.blade.php`/`tr/blog.blade.php` still emit `CollectionPage` schema, respectively — those are the only reasons the flags were removed (see Section 8).
- Do not remove the `robots`/`canonical_url`/`meta_keywords` per-page override columns on `Page`, and do not make the site-wide `index,follow` default in `master.blade.php`/`master-tr.blade.php` unconditional again (`<meta name="robots" content="@yield('robots', 'index,follow')">`) — a blank value on any given page must keep falling back to the global default exactly as before this feature.
- Do not build the Instagram Showcase (see Section 6) as a new Filament resource/page/CMS module — it must stay purely an extension of the existing `HomepageSettings` page (new keys in `TEXT_KEYS`/`FILE_KEYS` + collapsed `Section`s, one per row), per the explicit "do NOT create a new Homepage resource / Instagram module / CMS page" instruction behind this feature. Do not delete or rename `insta_url` — it is depended on (the shared Follow-button fallback for both rows and the fallback-overlay link) and now lives in Row 1's Showcase section (`instaShowcaseFields(..., includeProfileUrl: true)`). (The old `insta1_image`/`insta1_small_image`/`insta2_image`/`insta2_small_image` fields were deliberately removed per explicit user request — do not re-add them.) Do not change Row 1's embed field away from the historical key `insta_embed_url`, and keep passing it explicitly in the `form()` call — the helper's default would compute `insta_showcase_embed_url`, a dead key nothing reads, silently breaking Row 1 saves (the exact regression `tests/Feature/InstaShowcaseSaveTest.php` guards against). Do not use a third-party embed widget for the Instagram card — only Meta's official `instagram-media` blockquote + `embed.js`. Do not make `embed.js` load unconditionally or eagerly — it must stay behind both the row's `embed_url` check and the shared `IntersectionObserver` lazy-load gate (see Core Web Vitals Rules), and must always degrade to that row's fallback card (image/icon/"Watch on Instagram" button) rather than a blank space or a stuck spinner if the embed fails or times out. **Do not reintroduce `$row['enabled']` into the embed-vs-fallback content decision** (`@if($row['embed_url'])`, not `@if($row['enabled'] && $row['embed_url'])`) — that exact coupling was a real, user-reported bug (Row 1's embed silently required a separate toggle most admins never thought to flip; Row 2's fallback image was invisible unless the toggle was on). Do not make Row 2 (`insta_showcase2_*`) render only when `insta_showcase2_enabled` is truthy — the `@continue` gate must check `enabled` **or** `embed_url` **or** `fallback_image` (any one signals real content), so Row 2 appears automatically once the admin adds a photo or a URL, without also having to remember a separate switch, while an untouched install (all three falsy) still stays hidden — the same backward-compatibility guarantee, achieved without `enabled` being the sole gate. Do not revert `.insta-embed-wrap`'s `max-width:360px;min-height:640px` (~9:16) frame back to the original build's more-square `550×460` sizing, and do not raise the `640px` grid stacking breakpoint back toward `900px` — both the portrait frame and the "tablets stay side-by-side, only phones stack" breakpoint were explicit, direct user corrections; Row 2's columns are also intentionally reversed (`order`) in side-by-side mode to mirror Row 1's zigzag.
- Do not add the Training Articles parallax effect (`background-attachment:fixed` on `.section-news`) to `.counter`, `.result-section`, `.insta-showcase`, or `.insta-showcase--row2` — the user explicitly asked for this effect on the Training Articles section (`.section-news`) only ("نه به هر ۴ بخش اضافه نکن فقط بخش مقالات") and explicitly rejected applying it to all four background sections for visual consistency. If asked to extend it to another section later, that needs a fresh explicit request, not an inference from "the other backgrounds look similar."
- Do not reintroduce the Round-2 `.section-news-bg` transform-layer/JS workaround (see Section 6) for `.section-news` — it was explicitly reverted by the user in favor of native `background-attachment:fixed` (Round 3, the current, final state) once a properly sized `bg-articles.webp` (1822×863) made the underlying jitter/quality issue largely a non-issue. If a jitter complaint resurfaces, check the current background image's actual pixel dimensions against real viewport widths first — the root cause last time was an undersized image being upscaled via `background-size:cover`, not `background-attachment:fixed` in isolation — before re-adding transform-based JS complexity.
- Do not swap `bg-articles.jpg` (1349×637, the current, final file — see Section 6) for a higher-resolution replacement as a "quality improvement" — this was already tried (a 1822×863 AI-upscaled WebP, Round 3) and the user explicitly reverted it after directly comparing both and preferring the original JPG's look, on pure visual grounds unrelated to the technical upscaling argument (Round 4). Only change this file again if the user explicitly asks for a different image.
- Do not re-add a `FileUpload`-based media field or a searchable-`Select` media picker anywhere — every media/file selector in the CMS must use `App\Filament\Forms\Components\MediaPickerInput`, which opens the one unified Media Picker (Section 31). The old `App\Filament\Support\MediaLibraryUploads` helpers (`callback`/`pickFromLibraryAction`/`altHintAction`) were deleted; do not reintroduce them. New media fields are `MediaPickerInput::make(...)` (with `->onlyImages()` for image-only fields).
- Do not create a second Media Library implementation — `App\Filament\Concerns\InteractsWithMediaLibrary` is the single source of truth shared by both `App\Filament\Pages\MediaLibrary` (full page) and `App\Livewire\MediaPicker` (modal). Do not fork its folder/upload/filter/detail logic into a parallel copy (Section 31).
- Do not change `MediaPickerInput`'s stored value away from a plain `disk_path` string — that byte-identical value is what keeps the whole feature backward-compatible with every existing reader, stored value, and the settings pages' `mount()`/`save()`. Do not couple any caller to a concrete picker; go through the `open-media-picker`/`media-picker-selected` window-event contract (Section 31), which targets a field by its exact `statePath` (this is what makes repeater rows isolated — do not weaken it).
- Do not eagerly render the global `MediaPicker` modal's body — it must stay behind `@if($isOpen)` and be mounted once via the `AdminPanelProvider` `BODY_END` render hook, so it costs almost nothing on every panel page until opened (Section 31).
- Do not route Knowledge Base attachments through the unified Media Picker — they remain their own RAG document-ingestion pipeline (`KnowledgeEntryAttachment`, Section 27), deliberately separate; adopting the picker UI for them later must not change their storage/indexing model (Section 31).
- Do not extend the RichEditor's `insertContentFor()` to emit `<video>`/`<audio>`/`<iframe>`/card HTML without first opening the `Str::sanitizeHtml` (#73) allowlist for those tags — image (`<img>`) and document (`<a href>` download link) are the only forms the current sanitizer keeps; video/audio players and YouTube/Vimeo/Instagram/TikTok embeds are the separate, explicitly-scoped Video-SEO / External-Media phase (Section 31).

## 36. Future Development Guidelines
...
4. ~~Add JSON-LD structured data to the blog index and standalone `Page` records~~ — **done**: `Page` got `WebPage`/`FAQPage` schema on 2026-07-17, and the blog index got `CollectionPage`/`ItemList` schema on 2026-07-18 (`blog.blade.php`/`tr/blog.blade.php`). Every public template now emits JSON-LD; `SeoAuditService::missingSchema()` is a permanent zero-findings tripwire (see Section 8).
...
6. ~~Wire WebP delivery into the public templates~~ — **done** (2026-07-17, see Image Optimization Rules): every public featured-image `background-image` now prefers `Article`/`Page::optimized_image_url` (the DAM's WebP derivative), falling back to the original file when no `Media` row exists. What's still open, and still a separate, explicitly-scoped markup decision: **responsive `srcset`/`<picture>` delivery** (serving a smaller variant on mobile) — `Media::responsive_urls` is generated on every upload and ready, but wiring it in means either converting the `background-image` divs to real `<img>`/`<picture>` elements or adding per-breakpoint CSS media queries to the hand-rolled, pixel-matched styles (Brand Identity) — get sign-off first before touching that.
...
29. **Real generation for the Thumbnail/OG/Social image prompts** (Section 30) — the columns, the per-field text-generation UI, and the "no database redesign" architecture already exist; a real generation job for one of them is a thin variant of `GenerateHeroImage` (same `ProviderManager::generateImage()` call, a different resulting column/behavior instead of `image_path` — e.g. a dedicated `og_image_path` on `Article`/`Page`, or a separate `Media` row tagged for that purpose) plus one new UI button. This was explicitly deferred by the user's own confirmed scope ("even if only the Hero image is generated today") — build it only on a new, explicit request.
30. **Per-image cost tracking for the AI Image Pipeline** (Section 30) — `ai_usage_logs.estimated_cost_usd` is always `null` for `action_key: 'image_generation'` rows today, since `ai_provider_models`' pricing catalog is token-based, not per-image. A flat per-image price field on `AiProviderModel` (or a separate small catalog) would let `ProviderManager::logImageUsage()` estimate cost the same way `estimateCost()` does for text — worth building once real image-generation volume exists, same "don't build ahead of real usage" posture as item 17 above for text usage alerts.

When in doubt about whether a change fits this project's grain, re-read sections 2, 14, 33, and 34 above before proceeding.

## 37. Keeping CLAUDE.md Updated
